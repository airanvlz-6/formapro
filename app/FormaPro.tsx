...
const C = {
  ...
  warm: "#D4622A", warmLight: "#FDF0EB", tag: "#EDEAE4",
};
...
const buildPrompt = (cat: {id: string; titulo: string}, perfil: Record<string, string | string[]>) => {
...
export default function FormaPro() {
  ...
  const irACategoria = (catId: string) => {
    setCategoria(catId);
    setPregIdx(0);
    setRespuestas({});
    setSelMulti([]);
        ...
        <div className="fade-up" style={{ maxWidth: 500, width: "100%" }}>
          ...
          {pregActual.tipo === "opciones" && (
            <div style={{ display: "flex", flexWrap: "wrap", gap: 9, marginBottom: 28 }}>
              {pregActual.opciones?.map(op => (
                <Chip key={op} active={respuestas[pregActual.id] === op} color={accentColor} onClick={() => setRespuestas(p => ({ ...p, [pregActual.id]: op }))}>{op}</Chip>
              ))}
            </div>
          )}
          ...
          {pregActual.tipo === "texto" && (
            <textarea value={textoTemp} onChange={e => setTextoTemp(e.target.value)} rows={3} placeholder={pregActual.placeholder}
              style={{ width: "100%", border: `2px solid ${C.border}`, borderRadius: 14, padding: "13px 15px", fontSize: 14, color: C.ink, background: C.card, lineHeight: 1.65, marginBottom: 28, transition: "border-color 0.15s", fontFamily: "inherit" }}
