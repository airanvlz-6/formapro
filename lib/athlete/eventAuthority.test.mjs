...
test('UI offers optional date and confirms via dedicated form; generic creation drops event authority',()=>{
  const ui=readFileSync('components/TargetEventForm.tsx','utf8');assert.match(ui,/Aún no — continuar sin fecha/);assert.match(ui,/type="date"/);assert.match(ui,/operation, eventDate: date, token: context.token/);
