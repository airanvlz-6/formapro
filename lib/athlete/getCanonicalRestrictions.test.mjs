...
test('Builder selection and substitution share canonical areas without historical queries',()=>{
  ...
  assert.equal((text.match(/= canonicalRestrictions.areas/g)||[]).length,1);assert.match(text,/prepareSessionTrainingContract/);assert.match(text,/\}, canonicalRestrictions\)/);assert.match(text,/restriccionesReassessmentCheck = canonicalRestrictions.reassessments/);
