...
test('Builder delegates canonical restrictions to the shared server authority before generation',()=>{
  const text=branch('construir_sesion_dia').getText(root);assert.match(text,/generateTrainingSession/);
  const authority=readFileSync('lib/sports/sessionAuthority.ts','utf8');
  assert.equal((authority.match(/await getCanonicalRestrictions/g)||[]).length,2);
  assert.ok(authority.indexOf('await getCanonicalRestrictions') < authority.indexOf('await prepareSessionTrainingContract'));
  assert.ok(authority.indexOf('await prepareSessionTrainingContract') < authority.indexOf('await generateContractSession'));
...
test('actual Analyzer and Coach branches return identifiable 503 before generation on read failure',async()=>{
  for(const body of [branch('analizar_bloque_semana'),coach.thenStatement]) {
