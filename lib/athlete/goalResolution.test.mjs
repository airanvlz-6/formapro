...
test('3A conflict is retained even for aliases of one ID', () => {
  assert.equal(resolution({ objetivo_principal: 'crossfit', perfil: { objetivo_principal: 'CrossFit Open' } }).status, 'GOAL_CONFLICT');
});
...
test('CrossFit deload remains admitted with existing qualitative policy', () => {
  ...
  assert.equal(s.goal.id, 'crossfit'); assert.equal(s.volumeIntent, 'reduce');
  assert.ok(s.adaptations.some(a => a.id === 'recuperacion_activa'));
  assert.ok(s.adaptations.some(a => a.id === 'fuerza_maxima'));
});
...
function database(overrides = {}) {
  const tables = { usuarios: { codigo: 'fixture', modo_entrada: 'coach', categoria: 'hibrido', especialidad: 'hibrido_general',
    ...
    weekly_plan: [], session_modification_events: [], physiology_records: [], running_execution_records: [], athlete_state_events: [], athlete_coaching_notes: [] };
...
for (const [status, overrides] of [['GOAL_MISSING', {}], ['GOAL_UNSUPPORTED', { objetivo_principal: 'weightlifting' }],
  ['GOAL_CONFLICT', { objetivo_principal: 'crossfit', perfil: { objetivo_principal: 'half_marathon' } }]]) {
  test(`${status} stops normal Planner/Builder without writes`, async () => {
    ...
    assert.equal(r.ok, false); assert.equal(r.code, status === 'GOAL_UNSUPPORTED' ? 'STRATEGY_UNSUPPORTED' : status); assert.equal(r.goalRequirement.resolution.status, status);
...
test('explicit primary selection writes canonical source, archives conflicts and rereads database', async () => {
  ...
  assert.equal(db.tables.usuarios.objetivo_principal.descripcion, 'Media maratón');
  assert.deepEqual(db.tables.usuarios.objetivo_principal.resolution.previousPrimary, before.usuarios.objetivo_principal);
  assert.equal(db.tables.usuarios.objetivo_principal.resolution.previousProfileDeclarations.objetivo_detalle, undefined);
  assert.deepEqual(db.tables.usuarios.perfil, { objetivo_detalle: 'sub 90', objetivos_secundarios: ['hyrox'], objetivo_skill: 'snatch' });
});
...
test('unsupported answer and cancellation never mutate authority', async () => {
  ...
  assert.equal(r.code, 'GOAL_DECLARATION_REQUIRED'); assert.equal(db.writes.length, 0);
...
test('question and result serialize for Web and React Native with no browser dependency', async () => {
  ...
  assert.equal(q.question.options.length, 0); assert.equal(q.question.answerType, 'primary_goal_declaration');
  assert.doesNotMatch(Buffer.from(q.questionToken.split('.')[0], 'base64url').toString(), /objetivos_secundarios|snatch/);
});

test('detail remains contextual evidence and cannot create or conflict with primary', () => {
  const p = project({ objetivo_principal: 'crossfit', perfil: { objetivo_detalle: 'mejorar snatch y mantener carrera' } });
  assert.equal(resolve(p).status, 'GOAL_RESOLVED'); assert.equal(p.goals.detail[0].value, 'mejorar snatch y mantener carrera');
  assert.equal(resolution({ perfil: { objetivo_detalle: 'crossfit' } }).status, 'GOAL_MISSING');
});
for (const [a, b] of [['crossfit', 'half_marathon'], ['crossfit', 'mejorar snatch'], ['mejorar snatch', 'mantener carrera']]) {
  test(`question uses actual declarations: ${a} / ${b}`, async () => {
    const db = database({ objetivo_principal: a, perfil: { objetivo_principal: b } });
    const q = await answers.requireGoalAuthority(db, 'fixture');
    assert.deepEqual(plain(q.question.options.map(o => o.label)), [a, b]);
    assert.equal(q.question.options.length, 2); assert.ok(q.question.text.includes('Mi objetivo es:'));
    assert.equal(q.question.classification, a === 'mejorar snatch' ? 'needs_classification' : 'select_or_declare');
    const r = await answers.saveGoalAnswer(db, 'fixture', q.questionToken, 'mantener mi objetivo');
    assert.equal(r.resolved, false); assert.equal(r.code, 'GOAL_SELECTION_AMBIGUOUS'); assert.equal(db.writes.length, 0);
  });
}
test('explicit numbered supported selection persists then rereads resolved authority', async () => {
  const db = database({ objetivo_principal: 'crossfit', perfil: { objetivo_principal: 'half_marathon' } });
  const q = await answers.requireGoalAuthority(db, 'fixture');
  const r = await answers.saveGoalAnswer(db, 'fixture', q.questionToken, '2');
  assert.equal(r.resolved, true); assert.equal(r.resolution.canonicalGoalId, 'half_marathon'); assert.equal(db.userReads, 3);
});

test('explicit free description resolves through declared sport after write, retaining exact prose', async () => {
  const description = 'Open CrossFit Games 2027 - estándares Masters';
  const db = database({ especialidad: 'funcional_crossfit', objetivo_principal: 'crossfit', perfil: { objetivo_general: 'max_strength' } });
  const q = await answers.requireGoalAuthority(db, 'fixture');
  const r = await answers.saveGoalAnswer(db, 'fixture', q.questionToken, `Mi objetivo es: ${description}`);
  assert.equal(r.resolved, true); assert.equal(r.canPlanTowardDeclaredGoal, true);
  assert.equal(r.planningStrategy.strategyId, 'crossfit'); assert.equal(r.message, undefined);
  assert.equal(r.primaryGoal.descripcion, description); assert.equal(r.resolution.canonicalGoalId, null);
});
for (const answer of ['2', 'Mi objetivo es: quiero mejorar en halterofilia sin perder carrera']) {
  test(`unsupported explicit selection is persisted without granting Planner authority: ${answer}`, async () => {
    const db = database({ objetivo_principal: 'crossfit', perfil: { objetivo_principal: 'mejorar snatch', objetivo_detalle: 'contexto' } });
    const q = await answers.requireGoalAuthority(db, 'fixture'); const r = await answers.saveGoalAnswer(db, 'fixture', q.questionToken, answer);
    assert.equal(r.saved, true); assert.equal(r.resolved, false); assert.equal(r.resolution.status, 'GOAL_UNSUPPORTED');
    assert.equal(r.canPlanTowardDeclaredGoal, false); assert.equal(db.tables.usuarios.perfil.objetivo_detalle, 'contexto');
    let calls = 0; const plan = await load('../planning/prepareAllowedWeeklyPlanContract').planBoundedWeek(db, 'fixture', request, async () => { calls++; return ''; });
    assert.equal(plan.code, 'STRATEGY_UNSUPPORTED'); assert.equal(calls, 0);
  });
}
