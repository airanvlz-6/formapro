 ...
 */
export async function loadAthletePrescriptionContext(db: any, userCodigo: string, options: PrescriptionReadOptions, trace: GroundingTrace = silentGroundingTrace) {
  ...
  const rows = async (table: 'usuarios' | 'weekly_plan' | 'session_modification_events', query: PromiseLike<{ data: unknown; error: unknown }>, single = false) => trace.async(`athlete.${table}`, async () => {
  ...
  const [user, plans, modifications, restrictions, recovery, executions] = await Promise.all([
    ...
    trace.async('getCanonicalRestrictions', () => getCanonicalRestrictions(db, userCodigo, new Date(`${options.asOfDate}T12:00:00Z`))),
    options.recovery ?? trace.async('prepareRecoveryContext', () => prepareRecoveryContext(db, userCodigo, options.asOfDate)),
  ...
  const runningHistory = trace.sync('mergeRunningHistory', () => mergeRunningHistory(userCodigo, profile.workout_history, executions.history, options.asOfDate));
  runningDoseBaseline.prescriptionEvidence = {history:runningHistory,executions:executions.window,
    declarations:trace.sync('readMethodBaselines', () => readMethodBaselines(profile.perfil,userCodigo,options.runningHabitualInteraction)),
    restrictionsActive:restrictions.active,recovery,subjective:{score:null,date:options.asOfDate,source:'readiness_checkins'}};
  return trace.sync('buildAthleteContext', () => structuredClone({ ...projected, planningStrategy: trace.sync('resolvePlanningStrategy', () => resolvePlanningStrategy(projected)), userCodigo, asOfDate: options.asOfDate,
    runningHistory,
    runningDoseBaseline, runningDoseEvidenceAdmission: trace.sync('admitRunningDoseEvidence', () => admitRunningDoseEvidence(runningDoseBaseline)),
