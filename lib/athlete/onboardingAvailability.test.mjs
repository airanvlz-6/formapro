...
test('B/K: actual generation entry rereads after onboarding and asks before methodology agreement can generate', async () => {
  ...
  const fn = vm.runInNewContext(compile(`const run=${entry.getText(source)};run;`), {
    ...
    codigoUsuario: 'fixture', apiCall: async body => { if(body.action==='preparar_generacion_semana')return {ok:true,generation:{currentWeek:'2026-09-14',nextWeek:'2026-09-21'}};if(body.action==='check_week_closure')return {ok:true,yaCerrada:false};assert.equal(body.action,'obtener_confirmacion_disponibilidad'); return chat.readAvailabilityConfirmation(database, body.codigo); },
  ...
  assert.deepEqual(pending,[false,true]); assert.match(messages[0].content,/¿Sigue siendo correcta/);
