...
test('T7/T8 C2.1 projects labelled race distances; unsupported distances and malformed times stay absent', () => {
  for (const [id, metric, distance] of [['21k', 'halfMarathon', 21.0975], ['42k', 'marathon', 42.195]]) {
    assert.ok(marks.carrera.some(f => f.id === id));
    const p = project({ marcas_especificas: { [id]: '1:40:00' } });
    assert.equal(p.running.byMetric[metric].resolved.value.value, 6000);
    assert.equal(dose(p).references.find(r => r.metric === metric).value, 6000 / distance);
  }
  assert.equal(project({ marcas_especificas: { '50k': '4:00:00' } }).running.references.length, 0);
  for (const id of ['5k', '21k', '42k']) for (const value of ['24:99', 1470, 'fast'])
    assert.equal(project({ marcas_especificas: { [id]: value } }).running.references.length, 0);
});
