...
test('Carrera authenticated bootstrap persists canonical specialty', async () => {
  const f = fixture({ rows: [] });
  const result = await f.request({ intent: 'create_new_account', profile: { ...profile, categoria: 'carrera' } });
  assert.equal(result.status, 200);
  assert.equal(f.inserts[0].especialidad, 'carrera');
});
...
test('bootstrap projects nested profile and sets identity/email/privileges on server',async()=>{
  const f=fixture({rows:[]});const result=await f.request({intent:'create_new_account',profile:{...profile,admin:true},
    id:otherId,auth_user_id:otherId,authUserId:otherId,codigo:'STOLEN',email:'spoof@example.invalid',admin:true,premium:true, stripe_customer_id:'bad',password:'must-not-persist',confirmation:'must-not-persist'});
...
test('signup confirmation, callback SDK session, explicit bootstrap and resolution integration',async()=>{
  ...
  const auth={
    async signUp(input){assert.equal(input.options.emailRedirectTo,'https://www.forgeapp.es/auth/callback');return {data:{session:null},error:null};},
  ...
  assert.equal((await flow.signupForNewAccount(auth,'new@example.invalid','not-a-real-password','https://forgeapp.es','not-a-real-password')).state,'confirmation_required');
  assert.equal(f.inserts.length,0);
  assert.equal((await flow.authenticatedIdentityRequest(auth)).code,'AUTH_REQUIRED');
  const url='https://www.forgeapp.es/auth/callback?code=one-time-code';
...
test('invalid callback does not exchange or bootstrap',async()=>{
  ...
  const result=await flow.completeAuthCallback({exchangeCodeForSession(){calls++;}},'https://www.forgeapp.es/auth/callback?error=denied');
