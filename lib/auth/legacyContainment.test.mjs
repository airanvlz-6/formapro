...
// Invoke production POST with the identity boundary mocked as already authorized.
// All unexpected dependencies fail, including any database/auth/network access.
function runtime(db, apiKey = 'test') {
  const containment = sportsRuntime()('../auth/legacyContainment');
  const route = { exports: {} };
  ...
  vm.runInNewContext(compile(routeSource), {
    ...
    fetch: unexpected,
    require(name) {
      // These tests exercise the unchanged domain branches behind the identity boundary.
      // chatIdentity.test.mjs covers the real boundary and production POST rejection.
      if (name === '@/lib/auth/chatIdentity') return { authorizeChatRequest: req => req.json() };
      if (name === '@/lib/auth/athleteIdentity') return { IdentityError: class extends Error {} };
      if (name === '@/lib/chat/coachFirstFlag') return { coachFirstEnabled: () => false };
      ...
      if (name === 'next/server') return { NextRequest: class { constructor(url,init){this.body=JSON.parse(init.body);} async json(){return this.body;} }, NextResponse: {
...
for (const action of ['establecer_password_auth_admin', 'crear_usuario_desde_registro_movil',
  ...
  test(`${action}: disabled action cannot reach any privileged operation`, async () => {
    let calls = 0;
...
test('actual onboarding POST persists habitual discipline days and returns canonical distribution', async () => {
  const { db, writes } = profileDb();
  const days = ['lunes','miercoles','viernes','domingo'];
  const result = await runtime(db)({ action: 'guardar_usuario', datos: {
    codigo: 'LEGACY', categoria: 'carrera', especialidad: 'carrera', modo_entrada: 'planificacion',
    perfil: { dias_disponibles: days, duracion: 'Hasta 1h 30min' },
    distribucion_semanal: JSON.stringify({ disponibilidad: days }),
  } });
  assert.deepEqual(JSON.parse(writes[0].distribucion_semanal), { carrera: days });
  assert.equal(result.body.data.distribucion_semanal, writes[0].distribucion_semanal);
  assert.equal(writes[0].perfil.duracion, 'Hasta 1h 30min');
});
...
test('onboarding POST writes explicit captured primary while preserving profile detail and distance', async () => {
  const { db, writes } = profileDb();
  const perfil = { objetivo_detalle: 'mejorar tiempos en media maratón', distancia_objetivo: 'Media maratón (21K)' };
  const result = await runtime(db)({ action: 'guardar_usuario', datos: {
    categoria: 'carrera', especialidad: 'carrera', modo_entrada: 'planificacion', perfil,
    onboardingGoalCapture: { version: 1, source: 'onboarding_goal_question', field: 'objetivo_detalle', role: 'PRIMARY' },
  } });
  assert.deepEqual(writes[0].objetivo_principal, { descripcion: perfil.objetivo_detalle });
  assert.deepEqual(writes[0].perfil, perfil);
  assert.equal(Object.hasOwn(writes[0], 'onboardingGoalCapture'), false);
  assert.deepEqual(result.body.data.objetivo_principal, writes[0].objetivo_principal);
});
