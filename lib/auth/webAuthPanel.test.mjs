...
function fixture({ linked = true, session = true, initFailure = false, loginError = false, callbackError = false, legacyCodigo = 'FIXTURE-LINKED' } = {}) {
  ...
  const auth = {
    async signOut(){calls.push(['logout']);session=false;return {error:null};},
  ...
  const transport = async (url,options)=>{
    ...
    return Response.json(linked?{ok:true,athlete:{legacyCodigo}}:{ok:false,code:'ATHLETE_NOT_LINKED'});
  };
  const load=(file,react=hooks)=>{
    ...
    vm.runInNewContext(ts.transpileModule(readFileSync(file,'utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,jsx:ts.JsxEmit.ReactJSX,target:ts.ScriptTarget.ES2022}}).outputText,{
      ...
      window:{location:{replace:path=>calls.push(['navigate',path]),origin:'https://fixture.invalid',href:'https://fixture.invalid/auth/reset?code=fixture-code'},history:{replaceState(){}}},
...
test('AuthPanel server render includes visible form and diagnostic marker without initializing Auth',()=>{
  ...
  assert.match(html,/data-auth-panel="true"/);assert.match(html,/Entrar/);assert.match(html,/¿Olvidaste tu contraseña?/);
  assert.match(html,/class="auth-panel"/);
  assert.doesNotMatch(html,/Todavía accedo con código/);assert.equal(f.calls.length,0);
});
...
test('login resolves linked athlete through Bearer GET and never bootstraps',async()=>{
  const f=fixture({session:false,legacyCodigo:'060385'});let tree=f.render();input(tree,'Email','linked@example.invalid');input(tree,'Contraseña','fixture-password');
  await submit(f.render());tree=f.render();
  assert.deepEqual(f.calls.find(c=>c[0]==='navigate'),['navigate','/hoy']);
  assert.doesNotMatch(text(tree),/Continuar en Forge|Acceso verificado/);
  assert.equal((await f.auth.getSession()).data.session.access_token,'fixture-token');
...
test('signup mismatch blocks SDK; matching signup requests native confirmation and clears passwords',async()=>{
  const f=fixture();click(f.render(),'Crear cuenta');let tree=f.render();
...
test('forgot password delegates only to native resetPasswordForEmail with fixed callback',async()=>{
  const f=fixture();click(f.render(),'¿Olvidaste tu contraseña?');input(f.render(),'Email','account@example.invalid');
  await submit(f.render());const call=f.calls.find(c=>c[0]==='recovery');assert.equal(call[2].redirectTo,'https://www.forgeapp.es/auth/reset');
...
test('recovery callback unlocks update form; mismatch and missing session cannot update password',async()=>{
  ...
  assert.match(text(f.render({recovery:true})),/Inicia sesión con tu nueva contraseña/);
  assert.ok(f.calls.some(c=>c[0]==='logout'));
...
test('canonical entry with no session stays in login; existing session enters Hoy',async()=>{
  assert.match(readFileSync('app/page.tsx','utf8'),/<AuthPanel/);
  for(const session of [false,true]){
    const f=fixture({session});f.render();f.effects[0]();await new Promise(r=>setImmediate(r));
    assert.equal(f.calls.filter(c=>c[0]==='navigate').length,session?1:0);
    if(!session) assert.match(text(f.render()),/Entrar/);
  }
});
test('double login submit issues exactly one Auth request',async()=>{
  const f=fixture({session:false});let release;let requests=0;
  f.auth.signInWithPassword=async()=>{requests++;await new Promise(r=>{release=r;});return {error:{}};};
  const tree=f.render();const pending=submit(tree);await submit(tree);
  assert.equal(requests,1);assert.ok(nodes(f.render(),'button').every(b=>b.props.disabled));
  release();await pending;assert.match(text(f.render()),/No se pudo iniciar sesión/);
});
test('unlinked session logout returns to canonical access',async()=>{
  const f=fixture({linked:false});f.render();f.effects[0]();await new Promise(r=>setImmediate(r));
  await click(f.render(),'Cerrar sesión');assert.deepEqual(f.calls.find(c=>c[0]==='navigate'),['navigate','/']);
});
