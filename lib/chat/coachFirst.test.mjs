...
test('incident: JSON.parse of completion text is the SyntaxError before any tool; valid structure failures differ',async()=>{
  // These synthetic alternatives reproduce the same trace, not the unknown production body.
  ...
  for (const raw of invalidTexts) {
    ...
    }),error=>error.name==='SyntaxError' && error.stack.includes('JSON.parse'));
...
test('raw-invalid diagnostics expose only structural metadata from actual completion blocks',async()=>{
  ...
  for (const stopReason of ['max_tokens',privateText]) {
    ...
    const response=await handle(new Request('http://localhost/api/chat',{method:'POST',headers:{Authorization:'Bearer verified-token'},
      body:JSON.stringify({action:'coach_first',message:privateText,messageId:'message-raw-001'})}),()=>{throw Error('unexpected planning');});
    assert.equal(response.status,200);assert.equal((await response.json()).code,'COACH_FIRST_UNAVAILABLE');assert.equal(calls,1);
    const logs=observations.filter(e=>e.marker==='COACH_FIRST_RAW_INVALID');assert.equal(logs.length,1);
    assert.deepEqual(logs[0],{marker:'COACH_FIRST_RAW_INVALID',rawLength:raw.length,trimmedLength:raw.trim().length,
      empty:false,startsWithBrace:false,endsWithBrace:false,startsWithFence:true,endsWithFence:true,containsFence:true,
      leadingWhitespace:true,trailingWhitespace:true,contentBlockCount:3,textBlockCount:2,
      stopReason:stopReason==='max_tokens'?'max_tokens':'unknown',truncation:stopReason==='max_tokens'?'output_token_limit':'not_reported'});
    assert.ok(!JSON.stringify(logs).includes(privateText));assert.ok(!JSON.stringify(logs).includes('offline-placeholder'));
  }
});

test('raw diagnostics preserve original error even when observer fails; valid JSON emits nothing',async()=>{
  let original,observations=0;
  try{JSON.parse('');}catch(e){original=e;}
  await assert.rejects(loop.runCoachFirstLoop(input('offline'),{complete:async()=>'',dispatch:async()=>{throw Error('unexpected tool');},
    observeInvalidRaw:()=>{observations++;throw Error('logging failed');}}),e=>e.name===original.name && e.message===original.message);
  assert.equal(observations,1);
  await loop.runCoachFirstLoop(input('offline'),{complete:async()=>' {"answer":"ok","calls":[]} ',dispatch:async()=>{throw Error('unexpected tool');},
    observeInvalidRaw:()=>{observations++;}});
  assert.equal(observations,1);
});
