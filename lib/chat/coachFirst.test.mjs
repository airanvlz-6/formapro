...
test('generation argument reasons preserve all predicates, short-circuit order and public results', async t => {
  ...
  for (const fixture of fixtures) await t.test(fixture.name, async () => {
    ...
    const dispatch = tools.coachFirstTools(db, 'u', input(secret), 'turn',
      (a, id, diagnostic) => generation.generateCoachFirstWeek(db, 'u', a, id, today, stop, diagnostic), e => observations.push(e));
    // The dispatcher serializes calls for deduplication; throwing getters above test only the validator.
    const wireArgs = fixture.a && typeof fixture.a === 'object' ? Object.fromEntries(
      Object.entries(Object.getOwnPropertyDescriptors(fixture.a)).map(([key, descriptor]) =>
        [key, descriptor.get ? secret : descriptor.value])) : fixture.a;
    const result = await dispatch({ name: 'generate_week', arguments: wireArgs }, 1);
    assert.deepEqual(plain(result), { ...expected, operationId: 'turn:1' });
    assert.deepEqual(receipt(result), receipt({ ...baseline, operationId: 'turn:1' }));
    ...
    assert.deepEqual(plain(observations), [{ route: 'coach_first', policy: 'normal', tool: 'generate_week',
      authority: 'weekly_generation_authorities', status: 'rejected', operationId: 'turn:1',
...
test('generate_week rejection diagnostics are allowlisted and preserve the functional result', async t => {
  ...
  for (const fixture of cases) await t.test(fixture.name, async () => {
    ...
    const args = { week, includeToday: false, snapshotDigest: secret, extra: secret };
    const dispatch = tools.coachFirstTools(db, 'u', input(secret), 'turn', async (received, operationId) => {
      assert.equal(received, args); assert.equal(operationId, 'turn:1'); return fixture.result;
    }, e => observations.push(e));
    const result = await dispatch({ name: 'generate_week', arguments: args }, 1);
    // Existing dispatcher semantics: copy generation result and append the same operationId.
    assert.deepEqual(plain(result), { ...before, operationId: 'turn:1' });
...
test('generate_week diagnostics do not apply to other statuses or policy rejections', async () => {
  for (const status of ['committed', 'partial', 'unknown', 'conflict']) {
    ...
    assert.deepEqual(plain(await dispatch({ name: 'generate_week', arguments: {} }, 1)), { ...response, operationId: 'turn:1' });
