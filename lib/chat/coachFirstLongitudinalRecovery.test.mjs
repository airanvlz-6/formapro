...
const user = 'recovery-fixture', today = '2026-09-20', week = '2026-09-21';
...
function database() {
  ...
  const tables = {
    usuarios: [{ codigo: user, modo_entrada: 'coach', categoria: 'box', especialidad: 'crossfit',
      ...
      distribucion_semanal: { box: ['martes', 'jueves'] }, workout_history: [], athlete_development: [],
      ciclo_actual: { ...position, weekStart: undefined, planningWeekStart: oldWeek, positions: [position] } }],
    athlete_training_sources: [{ user_codigo: user, disciplina: 'box', owner: 'forge', activo: true, dias: ['martes', 'jueves'] }],
    ...
    week_closure_log: [{ id: 'old-closure', user_codigo: user, week_start: oldWeek, closed_at: today + 'T20:17:28Z' }],
...
test('Coach-first recovers closed legacy deload through real preflight, route Planner and cycle CAS', async t => {
  ...
  const confirmation = await load('chatAvailability').readAvailabilityConfirmation(db, user, week);
  assert.equal(confirmation.ok, true, JSON.stringify(confirmation));
  const result = await generation.generateCoachFirstWeek(db, user,
    { week, includeToday: false, snapshotDigest: confirmation.snapshotDigest }, 'recovery-operation', today,
