...
for (const turnIntent of [undefined, { version: 1, purpose: 'reintroduction', approach: 'conservative', volumeIntent: 'unspecified', intensityIntent: 'unspecified' }])
test('Coach-first recovers closed legacy deload through real preflight, route Planner and cycle CAS' + (turnIntent ? ' with turn intent' : ''), async t => {
  ...
  const provider = async (_url, request) => {
    ...
    if (turnIntent) {
      const projected = JSON.parse(prompt.split(marker)[1].split('\n')[0]);
      assert.deepEqual(projected.interpretation, turnIntent);
      assert.equal(projected.targetWeekStart, week);
    } else assert.ok(!prompt.includes(marker));
    let answer;
    if (prompt.startsWith('LONGITUDINAL_COACH_TRANSITION')) {
      const body = JSON.parse(request.body);
      assert.equal(body.tools.length, 1);
      assert.equal(body.tools[0].name, 'submit_longitudinal_decision');
      assert.equal(body.tools[0].input_schema.additionalProperties, false);
      assert.deepEqual(body.tool_choice, { type: 'tool', name: 'submit_longitudinal_decision', disable_parallel_tool_use: true });
      ...
      return { ok: true, json: async () => ({ stop_reason: 'tool_use', content: [
        { type: 'text', text: 'Ignored provider prose, not decision JSON.' },
        { type: 'tool_use', id: 'decision-fixture', name: 'submit_longitudinal_decision', input: answer },
      ] }) };
    } else if (prompt.includes('WEEKLY_CONTRACT:\n')) {
      assert.equal(JSON.parse(request.body).tools[0].name, 'submit_weekly_guidance');
      assert.equal(JSON.parse(request.body).tool_choice.name, 'submit_weekly_guidance');
      ...
      answer = { contractVersion: 3, contextDigest: contract.contextDigest,
        selections: Object.entries(contract.dayOptions).map(([day, options]) => {
          ...
          return train ? { day, state: 'TRAIN', discipline: 'box', guidance: {kind:'weekly_guidance',version:2,
            adaptation:'Upper body control',stimulus:'technical_push_density',patterns:['horizontal_push'],reason:'Synthetic adapted week.'} }
            : {day,state:'REST'};
        }) };
      return {ok:true,json:async()=>({stop_reason:'tool_use',content:[{type:'tool_use',id:'weekly-fixture',name:'submit_weekly_guidance',input:answer}]})};
    } else {
  ...
  const executePlanning = async (action, args, context) => {
      ...
      try {
        if (action === 'construir_sesion_dia') {
          assert.deepEqual(context.turnIntent ? plain(context.turnIntent.interpretation) : undefined, turnIntent);
  ...
  if (turnIntent) {
    // An independent request carrying the intent must reuse the already reserved position.
    const reserved = plain(db.tables.usuarios[0].ciclo_actual), callCount = providerCalls.length;
    const next = load('../chat/coachFirstTools').coachFirstTools(db, user, {
      message: 'Synthetic follow-up', messageId: 'second-message', timestamp: today + 'T21:00:00Z', timezone: 'Atlantic/Canary', conversation: [],
    }, 'second-turn', (args, id) => generation.generateCoachFirstWeek(db, user, args, id, today, executePlanning), () => {});
    const read = await next({ name: 'read_context', arguments: { resource: 'availability' } }, 1);
    await next({ name: 'read_context', arguments: { resource: 'planning' } }, 2);
    const second = await next({ name: 'generate_week', arguments: { availabilityReadId: read.availabilityReadId,
      snapshotDigest: read.data.snapshotDigest, includeToday: false, turnIntent } }, 3);
    assert.equal(second.code, 'BUILDER_NOT_ADMITTED');
    assert.deepEqual(providerCalls.slice(callCount), ['analyzer', 'weekly']);
    assert.deepEqual(db.tables.usuarios[0].ciclo_actual, reserved); assert.equal(db.writes.length, 1);
    assert.ok(!JSON.stringify(db.tables).includes('turnIntent'));
    assert.ok(!JSON.stringify(db.tables).includes('coach_turn_interpretation'));
  }
