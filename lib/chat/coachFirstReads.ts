...
export function coachFirstReads(db: any, user: string, today: string) {
  ...
  return { invalidate: () => cache.clear(), async read(a: any) {
    ...
    switch (a.resource) {
      ...
      case 'planning': data = { athlete: await loadAthletePrescriptionContext(db, user, { asOfDate: today }),
        reportedEvents: reportedEventProjection(await readCoachProfile(db, user)) }; break;
