...
const expected={
 success:[1,1,null], metadata_rejected:[1,1,null], plain_text:[1,1,null],
 ...
 empty:[2,0,'CHAT_ANSWER_INVALID'], bad_json:[2,0,'CHAT_RESPONSE_JSON_REQUIRED'], bad_answer:[2,0,'CHAT_ANSWER_INVALID'],
 review_rejected:[2,2,'CHAT_PROSE_UNGROUNDED'], review_json:[2,2,'CHAT_GROUNDING_REVIEW_INVALID'],
 review_failed:[2,2,'PRIVATE_ERROR'], repair_success:[2,1,null], prepare_failed:[0,0,'TypeError'],
};
...
for(const [mode,[generations,reviews,terminal]] of Object.entries(expected))test('Coach diagnostics ON/OFF preserve '+mode,async()=>{
 ...
 if(mode==='empty'||mode==='bad_answer')assert.ok(has('answer.validate','failure',2));
 if(mode==='bad_json')assert.ok(has('response.parse','rejected',2));
 if(mode==='review_json')assert.ok(has('review.parse','failure',2));
 if(mode==='review_rejected')assert.ok(has('review.admit','rejected',2));
 if(mode==='review_failed')assert.ok(has('review.provider.receive','failure',2));
