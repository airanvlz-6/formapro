...
test('wire schema preserves heterogeneous facts and every action kind without restricting nested vocabulary',()=>{
 ...
 assert.ok(matches(contract.COACH_REVIEW_FORMAT.schema,{supported:false,unsupportedClaims:['synthetic']}));
...
test('review remains mandatory and factual rejection remains terminal after two attempts',async()=>{
 let reviews=0;await assert.rejects(answerGroundedChat({facts:{sample:null},conversation:[]},'Reporte',async(s,m,o)=>{
  if(o.kind==='review'){reviews++;return '{"supported":false,"unsupportedClaims":["synthetic"]}';}
  return JSON.stringify(encode(envelope));
 }),/CHAT_PROSE_UNGROUNDED/);assert.equal(reviews,2);
});
