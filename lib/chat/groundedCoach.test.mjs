...
const events = [];
const load = sportsRuntime({ Error, Date: TestDate, console: { info: (...a) => events.push(a), log() {}, warn() {} }, process: { env: {
...
test('web and mobile route through shared backend, legacy extraction excludes assistant', () => {
  ...
  assert.match(client, /coachGrounding:true,codigo:codigoUsuario,coachMessage:texto/);
...
test('unclassified review rejection marks prose unverified without a second generation', async () => {
  ...
  assert.equal(proposals, 1); assert.equal(reviews, 1); assert.equal(result.extractionVerified, false); assert.equal(result.actionsAuthorized, false); assert.notEqual(result.answer, 'La restricción ha desaparecido y ya no hay nada pendiente.');
});
...
test('pain report does not disappear: retained as human conversation without diagnosis or permanent ban', async () => {
  ...
  const out = await load('../chat/runChatCoach').runChatCoach(db, 'u', message, async system => {
    ...
    assert.equal(f.HISTORICAL_CONTEXT.coachingKnowledge[0].kind,'discomfort_observation');
...
test('C: unstructured prose cannot bypass the generation contract', async () => {
  const ctx = await chat.loadChatGrounding(fixture(), 'u', today);
  await assert.rejects(chat.answerGroundedChat(ctx, '¿Qué harías después?', async () => coachingText), /CHAT_RESPONSE_JSON_REQUIRED/);
});
...
test('longitudinal report and 48h follow-up retain relevant sources without clinical state mutation', async () => {
  ...
  const r = await load('../chat/runChatCoach').runChatCoach(db, 'u', longitudinalReports[0], completionFor(longitudinalReports[0], {
    inspect(system) {
      const f = JSON.parse(system.split('FACTS (lectura autoritativa):\n')[1].split('\nAUTHORIZED_ACTION_RESULTS:')[0]);
      assert.ok(f.HISTORICAL_CONTEXT.longitudinal.entries.some(e => e.value?.includes('He reducido la carrera')));
      assert.ok(f.HISTORICAL_CONTEXT.longitudinal.entries.some(e => e.source === 'usuarios.workout_history'));
      assert.equal(f.restrictions.active, true);
    }
  }), today);
  assert.equal(r.answer, coachingText);
  await load('../chat/runChatCoach').runChatCoach(db, 'u', longitudinalReports[1], completionFor(longitudinalReports[1], {
    inspect(system, messages) {
      assert.ok(system.includes(longitudinalReports[0]) || messages.some(m=>m.content.includes(longitudinalReports[0])));
...
test('unsupported concrete claim is removed without regenerating the valid coaching remainder', async () => {
 const ctx = await chat.loadChatGrounding(fixture(), 'u', today);let count=0;
 const result = await chat.answerGroundedChat(ctx, 'Reporte.', async system => {
  if (system.startsWith('GROUNDING_REVIEW')) return JSON.stringify({supported:false,unsupportedClaims:[{quote:'Ya estás clínicamente recuperado.',kind:'unsupported_fact'}]});
  count++;return JSON.stringify({answer:'Ya estás clínicamente recuperado. Revisaría la siguiente sesión contigo.'});
 });assert.equal(count,1);assert.ok(!result.answer.includes('Ya estás clínicamente recuperado.'));assert.ok(result.answer.includes('Revisaría la siguiente sesión contigo.'));assert.equal(result.extractionVerified,false);assert.equal(result.actionsAuthorized,false);
});
...
test('grounding traces preserve context and DB effects with diagnostics on/off and isolate projection failure', async () => {
  const traceModule = load('../diagnostics/groundingTrace');
  const db = fixture(), before = JSON.stringify(db.tables);
  const silent = await chat.loadChatGrounding(db, 'u', today, 'PRIVATE_CURRENT_REPORT', traceModule.silentGroundingTrace);
  events.length = 0;
  const traced = await chat.loadChatGrounding(db, 'u', today, 'PRIVATE_CURRENT_REPORT');
  assert.deepEqual(plain(traced), plain(silent));
  assert.equal(JSON.stringify(db.tables), before); assert.equal(db.writes.length, 0);
  const rows = events.filter(e => e[0] === 'CHAT_GROUNDING_OPERATION').map(e => JSON.parse(e[1]));
  for (const stage of ['loadAthletePrescriptionContext','athlete.usuarios','athlete.weekly_plan','athlete.session_modification_events',
    'getCanonicalRestrictions','prepareRecoveryContext','readRunningExecutionViews','projectRunningDoseBaseline',
    'prescriptionHistorySummary','profile.read','plans.read','sources.read','resolveScope','loadEventContext',
    'loadWeeklyCoachingSupplement','buildSessionDoseContext','projectPlans','projectChatLongitudinal','projectConversation']) {
    assert.ok(rows.some(r => r.operation === stage && r.phase === 'start'), stage);
    assert.ok(rows.some(r => r.operation === stage && r.phase === 'success'), stage);
  }
  assert.ok(!JSON.stringify(rows).includes('PRIVATE_CURRENT_REPORT'));
  db.tables.weekly_plan[0].sessions.push(null);
  events.length = 0;
  await assert.rejects(chat.loadChatGrounding(db, 'u', today));
  const failed = events.filter(e => e[0] === 'CHAT_GROUNDING_OPERATION').map(e => JSON.parse(e[1])).filter(e => e.phase === 'failure');
  assert.equal(failed[0].operation, 'projectPlans'); assert.equal(failed[0].errorClass, 'TypeError');
});

test('grounding failure is located inside athlete loader and correlated to the existing pipeline', async () => {
  const db = fixture(), from = db.from.bind(db);
  db.from = table => {
    const q = from(table);
    if (table === 'session_modification_events') q.then = (yes, no) => Promise.resolve({data:null,error:{code:'42703',message:'PRIVATE_DB_ERROR'}}).then(yes,no);
    return q;
  };
  events.length = 0;
  await assert.rejects(load('../chat/runChatCoach').runChatCoach(db, 'u', 'PRIVATE_REPORT', async () => { throw new Error('synthetic completion failure'); }, today));
  const pipeline = JSON.parse(events.findLast(e => e[0] === 'CHAT_COACHING_PIPELINE')[1]);
  const rows = events.filter(e => e[0] === 'CHAT_GROUNDING_OPERATION').map(e => JSON.parse(e[1]));
  assert.ok(rows.every(r => r.runId === pipeline.runId && r.pass === 'initial'));
  assert.ok(rows.some(r => r.operation === 'athlete.session_modification_events' && r.phase === 'failure'
    && r.errorCode === 'PRESCRIPTION_CONTEXT_READ_FAILED:session_modification_events'));
  assert.equal(pipeline.groundingLoaded, false); assert.deepEqual(pipeline.failures, ['grounding']);
  assert.ok(!JSON.stringify(rows).includes('PRIVATE')); assert.equal(db.writes.length, 0);
});

// Real action authorities remain unchanged behind the structured wire codec.
for (const mode of ['availability', 'restriction', 'unknown_movement', 'mutation_rejected', 'persistence_failed']) {
 test('structured answer survives action authority: ' + mode, async () => {
  const db = mode === 'persistence_failed' ? database(activeFixture().tables, true) : activeFixture();
  const report = 'Quiero revisar la propuesta de hoy.';
  if (mode === 'availability') db.tables.usuarios[0].perfil.prescription_access = { [today]: { availability: 'unavailable' } };
  const a = alternative({ movement: mode === 'unknown_movement' ? 'movimiento_nuevo_del_coach' : 'bike' });
  const base = actionCompletion(report, [a], { actionSupported: !['restriction', 'mutation_rejected'].includes(mode) });
  const r = await load('../chat/runChatCoach').runChatCoach(db, 'u', report, async (system, messages, observation) => {
   const raw = await base(system, messages);
   if (observation?.kind !== 'generation') return raw;
   const {grounding, actions, ...rest} = JSON.parse(raw);
   return JSON.stringify({...rest, grounding: grounding.map(({fact,value}) => ({fact,valueJson:JSON.stringify(value)})), actionsJson: actions.map(a=>JSON.stringify(a))});
  }, today);
  assert.ok(r.answer.startsWith(coachingText)); assert.equal(r.pipeline.fallbackReason, null);
  assert.equal(r.pipeline.coachingResponseProduced, true);
  if (mode !== 'unknown_movement') {
   assert.notEqual(r.actions[0].status, 'committed');
   assert.match(r.answer, /no está confirmado|No se ha confirmado/);
   assert.ok(!r.answer.includes('Prescripción actualizada en Mi Plan.'));
  } else assert.equal(r.actions[0].status, 'committed');
 });
}

test('explicit restriction conflict requests confirmation without mutation or answer veto', async()=>{
 const db=activeFixture(),before=plain(db.tables.weekly_plan),report='Revisemos una posible reintroducción.';
 const base=actionCompletion(report,[alternative()]);
 const result=await load('../chat/runChatCoach').runChatCoach(db,'u',report,async(s,m,o)=>s.startsWith('CHAT_ACTION_REVIEW')?JSON.stringify({supported:false,conflict:'restriction'}):base(s,m,o),today);
 assert.equal(result.actions[0].status,'confirmation_required');assert.ok(result.answer.startsWith(coachingText));assert.match(result.answer,/¿Confirmas/);assert.deepEqual(plain(db.tables.weekly_plan),before);
});
test('incident regression: longitudinal grounding, new subjective evidence, mismatch and review dissent keep one generation',async()=>{
 const db=fixture(),report='Llevo varios días con digestión pesada y estoy descansando peor.';
 let generations=0,reviews=0;
 const result=await load('../chat/runChatCoach').runChatCoach(db,'u',report,async(s,m,o)=>{
  if(o?.kind==='review'){reviews++;assert.equal(JSON.parse(m[0].content).userReport,report);return JSON.stringify({supported:false,unsupportedClaims:[{quote:'Reduciría la exigencia esta semana.',kind:'recommendation'}]});}
  generations++;return JSON.stringify({answer:'Con lo que describes, reduciría la exigencia esta semana.',grounding:[{fact:'today',value:'incorrect'}],evidence:[],interpretation:'',decision:'',actions:[]});
 },today);
 assert.equal(generations,1);assert.equal(reviews,1);assert.equal(result.pipeline.groundingLoaded,true);assert.ok(result.pipeline.relevantHistoryCount>0);assert.equal(result.pipeline.coachingResponseProduced,true);assert.equal(result.pipeline.fallbackReason,null);assert.match(result.answer,/reduciría la exigencia/);assert.equal(result.pipeline.verifiedFactCount,0);
});

test('unsupported HRV claim cannot authorize an action or persisted learning; coaching remainder survives',async()=>{
 const db=activeFixture(),before=plain(db.tables.weekly_plan),report='Quiero revisar la semana.';
 let generations=0;
 const result=await load('../chat/runChatCoach').runChatCoach(db,'u',report,async(s,m,o)=>{
  if(o?.kind==='review')return JSON.stringify({supported:false,unsupportedClaims:[{quote:'Tu HRV cayó un 25%.',kind:'unsupported_fact'}]});
  generations++;return JSON.stringify({answer:'Tu HRV cayó un 25%. Propongo ajustar la sesión contigo.',grounding:[],evidence:[],interpretation:'',decision:'',actions:[alternative()]});
 },today);
 assert.equal(generations,1);assert.ok(!result.answer.includes('Tu HRV cayó'));assert.match(result.answer,/Propongo ajustar/);assert.equal(result.actions[0].code,'CHAT_ACTION_FACTS_UNVERIFIED');assert.equal(result.pipeline.mutationSucceeded,false);assert.equal(result.pipeline.verifiedFactCount,0);assert.deepEqual(plain(db.tables.weekly_plan),before);
 assert.ok(!JSON.stringify(db.tables.usuarios[0].historial).includes('Tu HRV cayó'));
});

test('canonical explicit restriction yields confirmation_required before any plan write',async()=>{
 const db=activeFixture();db.tables.athlete_coaching_notes=[{id:'r',user_codigo:'u',movement:'box',issue:'Restricción explícita',constraint_level:'hard',status:'pending',valid_until:null,source:'athlete'}];
 const before=plain(db.tables.weekly_plan),report='Quiero discutir esta propuesta.';
 const r=await load('../chat/runChatCoach').runChatCoach(db,'u',report,actionCompletion(report,[alternative()]),today);
 assert.equal(r.actions[0].status,'confirmation_required');assert.match(r.answer,/¿Confirmas/);assert.deepEqual(plain(db.tables.weekly_plan),before);
});

for (const reviewKind of ['recommendation', 'interpretation', 'legacy_unsupported']) {
 test('no-week longitudinal Coach: valid answer, one generation, zero mutations / ' + reviewKind, async () => {
  const db = fixture();
  db.tables.weekly_plan = [];
  db.tables.usuarios[0].historial = [...longitudinalHistory];
  db.tables.usuarios[0].workout_history = [{fecha:'2026-09-12',disciplina:'box',descripcion:'Trabajo técnico de fuerza registrado, sin dosis cuantificada.'}];
  const profileBefore = plain(db.tables.usuarios[0].perfil);
  const report = 'Llevo varios dias con el estómago pesado y mala digestion, ademas, mi hijo vuelve a tener mocos y por las noches no descansamos mucho en casa porque se despierta por la tos. Como afrontamos esta nueva semana de entrenos?¿';
  const coachAnswer = 'Con el descanso y la digestión que describes, reduciría la exigencia al empezar la semana. Tu historial recoge trabajo técnico de fuerza tolerable: puede orientar la elección, sin asumir cómo responderás ahora. Todavía no hay una semana generada que modificar; plantearía las primeras exposiciones según cómo te encuentres.';
  let generations = 0, reviews = 0, observedFacts;
  events.length = 0;
  const result = await load('../chat/runChatCoach').runChatCoach(db, 'u', report, async (system, messages, observation) => {
   if (observation?.kind === 'review') {
    reviews++;
    assert.equal(JSON.parse(messages[0].content).userReport, report);
    return JSON.stringify({supported:false,unsupportedClaims:reviewKind === 'legacy_unsupported'
      ? ['recommendation not literally in canonical facts'] : [{quote:'reduciría la exigencia al empezar la semana.',kind:reviewKind}]});
   }
   assert.equal(observation?.kind, 'generation', 'No action or learning provider call is needed');
   generations++;
   observedFacts = JSON.parse(system.split('FACTS (lectura autoritativa):\n')[1].split('\nAUTHORIZED_ACTION_RESULTS:')[0]);
   assert.equal(messages.at(-1).content, report);
   assert.ok(messages.length <= 7);
   assert.equal(observedFacts.CURRENT_WEEK.status, 'NONE');
   assert.deepEqual(observedFacts.FUTURE_PLANS, []);
   assert.ok(observedFacts.goal);
   assert.ok(observedFacts.ownership);
   assert.ok(observedFacts.readiness);
   assert.ok(observedFacts.restrictions);
   assert.ok(JSON.stringify(observedFacts.RECENT_EXECUTED_TRAINING).includes('Trabajo técnico de fuerza'));
   assert.ok(messages.some(m => m.role === 'user' && m.content.includes('sin reacción posterior')));
   assert.deepEqual(plain(observation.outputFormat), plain(load('../chat/coachOutputContract').COACH_GENERATION_FORMAT));
   return JSON.stringify({answer:coachAnswer,grounding:[{fact:'today',valueJson:JSON.stringify('incorrect metadata date')}],evidence:[],interpretation:'Ajuste provisional a evidencia actual.',decision:'Reducir exigencia inicial.',actionsJson:[]});
  }, today);
  assert.equal(generations, 1); assert.equal(reviews, 1);
  assert.equal(result.pipeline.groundingLoaded, true);
  assert.equal(result.pipeline.relevantHistoryCount, observedFacts.HISTORICAL_CONTEXT.longitudinal.entries.length + longitudinalHistory.length);
  assert.ok(result.pipeline.relevantHistoryCount > 0);
  assert.equal(result.pipeline.coachingResponseProduced, true);
  assert.equal(result.pipeline.fallbackReason, null);
  assert.equal(result.pipeline.mutationAttempted, false);
  assert.equal(result.pipeline.mutationSucceeded, false);
  assert.equal(result.pipeline.verifiedFactCount, 0);
  assert.ok(result.answer.includes(coachAnswer));
  assert.deepEqual(plain(result.actions), []);
  assert.equal(result.adaptation.status, 'not_needed');
  assert.ok(!result.answer.includes('¿Confirmas'));
  assert.deepEqual(db.tables.weekly_plan, []);
  assert.deepEqual(plain(db.tables.usuarios[0].perfil), profileBefore);
  assert.ok(db.writes.every(w => w.table === 'usuarios' && Object.keys(w.patch).every(k => k === 'historial')), 'Only conversation history may be saved');
  const operations = events.filter(e => e[0] === 'CHAT_COACH_OPERATION').map(e => JSON.parse(e[1])).filter(e => e.runId === result.pipeline.runId);
  for (const operation of ['response.parse','answer.validate','answerGroundedChat']) assert.ok(operations.some(e => e.operation === operation && e.phase === 'success'), operation);
  assert.ok(operations.some(e => e.operation === 'metadata.validate' && e.errorCode === 'CHAT_CANONICAL_FACT_MISMATCH'));
  const contexts = events.filter(e => e[0] === 'CHAT_COACH_CONTEXT').map(e => JSON.parse(e[1]));
  assert.ok(contexts.length > 0 && contexts.every(c => c.plans === 0));
 });
}

test('Generation temporal projection partitions current/future/history without changing authority facts', async () => {
 const db=fixture(), base=structuredClone(db.tables.weekly_plan[0]);
 db.tables.weekly_plan=[{...structuredClone(base),week_start:'2026-09-07',revision:2},
  {...structuredClone(base),week_objective:'Current objective',revision:3},
  {...structuredClone(base),week_start:'2026-09-21',revision:4,week_objective:'Future objective'}];
 db.tables.usuarios[0].historial=[...longitudinalHistory];
 db.tables.usuarios[0].workout_history=[{fecha:'2026-09-10',notas:'Historical activity'}];
 const context=await chat.loadChatGrounding(db,'u',today), before=plain(context);
 const projected=plain(chat.projectChatGenerationFacts(context));
 const current=context.facts.plan.find(p=>p.weekStart===today);
 assert.deepEqual(projected.CURRENT_WEEK,{...plain(current),status:'ACTIVE'});
 assert.deepEqual(projected.FUTURE_PLANS,plain(context.facts.plan.filter(p=>p.weekStart>today)));
 assert.ok(!JSON.stringify(projected.CURRENT_WEEK).includes('Future objective'));
 assert.ok(!JSON.stringify(projected.HISTORICAL_CONTEXT).includes('Future objective'));
 assert.equal(projected.plan,undefined); assert.equal(projected.history,undefined); assert.equal(projected.longitudinal,undefined);
 for(const key of ['exposure','recentFrequency','modifications']) assert.deepEqual(projected.HISTORICAL_CONTEXT[key],plain(context.facts.history[key]),key);
 assert.equal(projected.HISTORICAL_CONTEXT.completedSessions,undefined);
 assert.equal(projected.HISTORICAL_CONTEXT.runningExecution.records,undefined);
 assert.deepEqual(projected.HISTORICAL_CONTEXT.runningExecution.summaries,plain(context.facts.runningExecution.summaries));
 assert.deepEqual(projected.HISTORICAL_CONTEXT.coachingKnowledge,plain(context.facts.coachingKnowledge));
 assert.ok(projected.HISTORICAL_CONTEXT.longitudinal.entries.every(e=>!context.temporalConversation.slice(-6).some(m=>e.provenance?.index===m.provenance.index&&e.provenance?.source===m.provenance.source)));
 assert.deepEqual(projected.HISTORICAL_CONTEXT.advisory,plain(context.advisory));
 assert.deepEqual(projected.availability,plain(context.facts.availability));
 assert.deepEqual(plain(context),before);
 let calls=0;
 const result=await chat.answerGroundedChat(context,'¿Cómo planteamos la semana?',async(system,messages,observation)=>{
  if(observation.kind==='review') {
   assert.ok(system.includes('FACTS:'+JSON.stringify(context.facts)),'Reviewer keeps its original context');
   return '{"supported":true,"unsupportedClaims":[]}';
  }
  calls++;
  const received=JSON.parse(system.split('FACTS (lectura autoritativa):\n')[1].split('\nAUTHORIZED_ACTION_RESULTS:')[0]);
  assert.deepEqual(received,projected);
  assert.ok(system.includes('FACTS.CURRENT_WEEK.sessions o FACTS.FUTURE_PLANS[].sessions'));
  assert.ok(!system.includes('FACTS.plan'));
  return JSON.stringify({answer:'Podemos revisar las primeras exposiciones.',grounding:[{fact:'CURRENT_WEEK',valueJson:JSON.stringify(received.CURRENT_WEEK)}],evidence:[],interpretation:'',decision:'',actionsJson:[]});
 });
 assert.equal(calls,1);assert.equal(result.extractionVerified,true);
});

test('no current week with future plans is NONE, not ACTIVE; future weeks keep their sessions',async()=>{
 const db=fixture();db.tables.weekly_plan[0].week_start='2026-09-21';
 const context=await chat.loadChatGrounding(db,'u',today);
 const projected=chat.projectChatGenerationFacts(context);
 assert.equal(projected.CURRENT_WEEK.status,'NONE');assert.equal(projected.CURRENT_WEEK.weekStart,today);
 assert.equal(projected.CURRENT_WEEK.sessions,undefined);
 assert.deepEqual(plain(projected.FUTURE_PLANS),plain(context.facts.plan));
 assert.deepEqual(plain(projected.HISTORICAL_CONTEXT.prescriptions),[]);
});

test('historical Tuesday/Thursday/Saturday and habitual days cannot populate CURRENT_WEEK',async()=>{
 const db=fixture();db.tables.weekly_plan[0].week_start='2026-09-07';
 db.tables.weekly_plan[0].sessions=db.tables.weekly_plan[0].sessions.filter(s=>['martes','jueves','sabado'].includes(s.dia));
 for(const s of db.tables.weekly_plan[0].sessions) Object.assign(s,{tipo:'box',titulo:'Historical '+s.dia,descripcion:'Historical prescription',completada:false});
 db.tables.usuarios[0].distribucion_semanal=JSON.stringify({box:['martes','jueves','sabado']});
 db.tables.usuarios[0].historial=[{role:'assistant',content:'El sábado lo dejamos como estaba.'}];
 const context=await chat.loadChatGrounding(db,'u',today), p=chat.projectChatGenerationFacts(context);
 assert.equal(p.CURRENT_WEEK.status,'NONE');assert.deepEqual(plain(p.FUTURE_PLANS),[]);
 assert.equal(p.CURRENT_WEEK.sessions,undefined);
 assert.deepEqual(plain(p.PAST_PRESCRIPTIONS_WITHOUT_EXECUTION).map(s=>[s.day,s.date,s.source,s.temporal,s.factualState]),[
  ['sabado','2026-09-12','weekly_plan.sessions','PAST','PLANNED_ONLY'],
  ['jueves','2026-09-10','weekly_plan.sessions','PAST','PLANNED_ONLY'],
  ['martes','2026-09-08','weekly_plan.sessions','PAST','PLANNED_ONLY'],
 ]);
 assert.ok(context.temporalConversation.some(e=>e.role==='assistant'&&e.temporal.status==='UNKNOWN'));
 assert.equal(p.availability.habitual,db.tables.usuarios[0].distribucion_semanal);
});

test('UNKNOWN is distinct from a confirmed empty read, including invalid dates and ambiguous plans',()=>{
 const contexts=[
  {facts:{today,plan:[]}},
  {facts:{today,plan:[],contextStatus:'UNKNOWN'},planRead:{status:'SUCCESS',weekStart:today}},
  {facts:{today,plan:[]},planRead:{status:'SUCCESS',weekStart:'2026-09-07'}},
  {facts:{today:'invalid',plan:[]},planRead:{status:'SUCCESS',weekStart:today}},
  {facts:{today},planRead:{status:'SUCCESS',weekStart:today}},
  {facts:{today,plan:[{weekStart:today,sessions:[]},{weekStart:today,sessions:[]}]},planRead:{status:'SUCCESS',weekStart:today}},
  {facts:{today,plan:[{weekStart:'invalid',sessions:[]}]},planRead:{status:'SUCCESS',weekStart:today}},
 ];
 for(const context of contexts)assert.equal(chat.projectChatGenerationFacts({...context,conversation:[]}).CURRENT_WEEK.status,'UNKNOWN');
 const invalid=contexts.at(-1);assert.deepEqual(plain(chat.projectChatGenerationFacts({...invalid,conversation:[]}).CURRENT_WEEK.unresolvedPlans),invalid.facts.plan);
});

test('actual plan read failure projects UNKNOWN to Generation and still permits coaching',async()=>{
 const db=fixture(), original=db.from;
 db.from=table=>{const q=original(table);if(table==='weekly_plan')q.then=(yes,no)=>Promise.resolve({data:null,error:{code:'SYNTHETIC'}}).then(yes,no);return q;};
 let generations=0;
 const result=await load('../chat/runChatCoach').runChatCoach(db,'u','¿Cómo planteamos esta semana?',async(system,messages,observation)=>{
  if(observation.kind==='review')return '{"supported":true,"unsupportedClaims":[]}';
  generations++;
  const facts=JSON.parse(system.split('FACTS (lectura autoritativa):\n')[1].split('\nAUTHORIZED_ACTION_RESULTS:')[0]);
  assert.equal(facts.CURRENT_WEEK.status,'UNKNOWN');
  return JSON.stringify({answer:'Podemos razonar provisionalmente con tu reporte.',grounding:[{fact:'CURRENT_WEEK',valueJson:JSON.stringify(facts.CURRENT_WEEK)}],evidence:[],interpretation:'',decision:'',actionsJson:[]});
 },today);
 assert.equal(generations,1);assert.equal(result.pipeline.groundingLoaded,false);assert.equal(result.pipeline.coachingResponseProduced,true);assert.equal(result.pipeline.fallbackReason,null);
});

test('Generation anchors conversation by snapshot provenance without losing older or repeated messages', async()=>{
 const db=fixture();db.tables.weekly_plan[0].week_start='2026-09-07';
 const repeated='El martes técnica, miércoles bici Z2 40 min, jueves gimnásticos y sábado potencia esta semana.';
 db.tables.usuarios[0].historial=[{role:'assistant',content:repeated,timestamp:'2026-09-07T10:00:00Z'},
  ...Array.from({length:6},(_,i)=>({role:i===5?'assistant':'user',content:i===5?repeated:'Contexto '+i,
   ...(i===0?{timestamp:'2026-09-12T09:00:00+01:00'}:{})}))];
 const c=await chat.loadChatGrounding(db,'u',today), p=chat.projectChatGenerationFacts(c);
 const entries=p.HISTORICAL_CONTEXT.longitudinal.entries.filter(e=>e.provenance);
 assert.deepEqual(plain(entries.map(e=>e.provenance.index)),[0]);
 assert.equal(entries[0].value,repeated);assert.equal(entries[0].temporal.timestamp,'2026-09-07T10:00:00Z');
 assert.equal(p.CURRENT_WEEK.status,'NONE');assert.ok(p.HISTORICAL_CONTEXT.prescriptions.every(s=>s.temporal==='PAST'));
 await chat.answerGroundedChat(c,'¿Cómo seguimos?',async(system,messages,observation)=>{
  if(observation.kind==='review')return '{"supported":true,"unsupportedClaims":[]}';
  assert.equal(messages.length,7);
  const dated=JSON.parse(messages[0].content), unknown=JSON.parse(messages[5].content);
  assert.equal(dated.temporal.timestamp,'2026-09-12T09:00:00+01:00');assert.equal(dated.temporal.status,'KNOWN');
  assert.equal(unknown.temporal.status,'UNKNOWN');assert.equal(unknown.temporal.timestamp,null);
  assert.equal(unknown.content,repeated);assert.equal(unknown.provenance.index,6);
  return JSON.stringify({answer:'Podemos plantear una nueva semana.',grounding:[{fact:'CURRENT_WEEK',valueJson:JSON.stringify(p.CURRENT_WEEK)}],evidence:[],interpretation:'',decision:'',actionsJson:[]});
 });
});

test('conversation dates cannot be inferred from text, invalid metadata or conflicting timestamps',()=>{
 const project=load('../chat/conversationEvidence').contextualConversation;
 const dated={role:'user',content:'El martes esta semana',timestamp:'2026-09-12T09:00:00+01:00'};
 assert.equal(chat.conversationOnly([dated])[0].timestamp,dated.timestamp);
 assert.equal(project(chat.conversationOnly([dated]))[0].temporal.timestamp,dated.timestamp);
 for(const metadata of [{},{timestamp:'yesterday'},{timestamp:'2026-09-14T10:00:00Z',createdAt:'2026-09-13T10:00:00Z'}]){
  const m=project([{role:'user',content:'Esta semana [Fecha actual del sistema: 2026-09-14]',...metadata}])[0];
  assert.equal(m.temporal.status,'UNKNOWN');assert.equal(m.temporal.timestamp,null);
 }
});

test('CURRENT_CYCLE uses existing exact-week authority, independently of CURRENT_WEEK',async()=>{
 for(const stored of [
  {bloque:'deload',semana:1,planningWeekStart:today},
  {bloque:'future',semana:2,planningWeekStart:'2026-09-21',positions:[{weekStart:today,bloque:'deload',semana:1}]},
  {bloque:'deload',semana:1,updated_at:today},
  {bloque:'deload',semana:1,planningWeekStart:'2026-09-07'},
 ]){
  const db=fixture();db.tables.weekly_plan=[];db.tables.usuarios[0].ciclo_actual=stored;
  const c=await chat.loadChatGrounding(db,'u',today),p=chat.projectChatGenerationFacts(c);
  assert.equal(p.CURRENT_WEEK.status,'NONE');assert.equal(p.cycle,undefined);
  assert.deepEqual(plain(p.HISTORICAL_CONTEXT.declaredCycle.stored),stored);
  const active=stored.planningWeekStart===today||stored.positions?.some(s=>s.weekStart===today);
  assert.equal(p.CURRENT_CYCLE.status,active?'ACTIVE':'UNKNOWN');
  if(active){assert.equal(p.CURRENT_CYCLE.weekStart,today);assert.equal(p.CURRENT_CYCLE.bloque,'deload');assert.equal(p.CURRENT_CYCLE.semana,1);}
 }
});

function unexecutedWeekFixture() {
 const db=fixture();
 db.tables.weekly_plan[0].week_start='2026-09-07';
 db.tables.weekly_plan[0].sessions=[
  ['martes','halterofilia técnica','box',null],['miercoles','bici Z2 40 min','bici',40],
  ['jueves','gimnásticos','box',null],['sabado','potencia','box',null],
 ].map(([dia,titulo,tipo,duracion_min],i)=>({session_id:'past-'+i,dia,titulo,tipo,duracion_min,completada:false,descripcion:titulo}));
 db.tables.usuarios[0].workout_history=[{workout_id:'unplanned-run',fecha:'2026-09-11',tipo:'carrera',duracion:'35 min',distancia:'5 km',sensacion:'bien'}];
 return db;
}

test('Generation: four past unexecuted prescriptions, one unplanned run, no current week',async()=>{
 const db=unexecutedWeekFixture(),c=await chat.loadChatGrounding(db,'u',today),before=plain(c);
 const p=plain(chat.projectChatGenerationFacts(c));
 assert.equal(p.CURRENT_WEEK.status,'NONE');assert.deepEqual(p.FUTURE_PLANS,[]);
 assert.equal(p.RECENT_EXECUTED_TRAINING.length,1);
 const run=p.RECENT_EXECUTED_TRAINING[0];
 assert.equal(run.discipline,'carrera');assert.equal(run.date,'2026-09-11');assert.equal(run.provenance,'LEGACY_STRUCTURED');
 assert.equal(run.quantities.totalDurationSeconds.value,2100);assert.equal(run.quantities.distanceMeters.value,5000);
 assert.equal(run.quantities.totalDurationSeconds.confidence,'LEGACY_STRUCTURED_UNVERIFIED');
 assert.equal(p.PAST_PRESCRIPTIONS_WITHOUT_EXECUTION.length,4);
 assert.deepEqual(p.PAST_PRESCRIPTIONS_WITHOUT_EXECUTION.map(s=>s.prescription.title),['potencia','gimnásticos','bici Z2 40 min','halterofilia técnica']);
 assert.ok(p.PAST_PRESCRIPTIONS_WITHOUT_EXECUTION.every(s=>s.temporal==='PAST'&&s.executionStatus==='NO_EXECUTION_RECORDED'));
 assert.ok(!JSON.stringify(p).includes('CONFIRMED_NOT_PERFORMED'));
 assert.equal(p.HISTORICAL_CONTEXT.prescriptions.length,0);assert.deepEqual(p.RESPONSE,[]);
 assert.equal(p.HISTORICAL_CONTEXT.legacyResponseEvidence[0].value,'bien');
 assert.equal(p.RECENT_EXECUTED_TRAINING[0].sensation,undefined);
 assert.deepEqual(plain(c),before);assert.equal(db.writes.length,0);
 let calls=0;
 await chat.answerGroundedChat(c,'¿Cómo afrontamos esta semana?',async(system,messages,observation)=>{
  if(observation.kind==='review') {assert.ok(system.includes('FACTS:'+JSON.stringify(c.facts)));return '{"supported":true,"unsupportedClaims":[]}';}
  calls++;const received=JSON.parse(system.split('FACTS (lectura autoritativa):\n')[1].split('\nAUTHORIZED_ACTION_RESULTS:')[0]);
  assert.deepEqual(received,p);
  return JSON.stringify({answer:'Podemos plantear la semana.',grounding:[{fact:'CURRENT_WEEK',valueJson:JSON.stringify(p.CURRENT_WEEK)}],evidence:[],interpretation:'',decision:'',actionsJson:[]});
 });
 assert.equal(calls,1);
});

test('completed plan preserves occurrence but never copies prescribed quantity into executed training',async()=>{
 const db=unexecutedWeekFixture();db.tables.usuarios[0].workout_history=[];
 Object.assign(db.tables.weekly_plan[0].sessions[1],{completada:true,descripcion_real:'Hice trabajo parcial',duracion_min:90});
 const c=await chat.loadChatGrounding(db,'u',today),p=chat.projectChatGenerationFacts(c);
 assert.equal(p.RECENT_EXECUTED_TRAINING.length,1);assert.equal(p.RECENT_EXECUTED_TRAINING[0].quantities,null);
 assert.equal(p.RECENT_EXECUTED_TRAINING[0].evidenceStatus,'COMPLETION_RECORDED');
 assert.equal(p.PAST_PRESCRIPTIONS_WITHOUT_EXECUTION.length,3);
 assert.equal(p.HISTORICAL_CONTEXT.prescriptions[0].prescription.durationMinutes,90);
 assert.equal(p.HISTORICAL_CONTEXT.prescriptions[0].executionRef,p.RECENT_EXECUTED_TRAINING[0].executionId);
});

test('Generation reuses modern/legacy reconciliation and explicit plan association without duplicate execution',async()=>{
 const db=unexecutedWeekFixture();
 const s=db.tables.weekly_plan[0].sessions[0];Object.assign(s,{tipo:'carrera',completada:true,descripcion_real:'Carrera reportada'});
 db.tables.usuarios[0].workout_history=[{workout_id:'real-run',fecha:'2026-09-08',tipo:'carrera',duracion:'90 min',sensacion:'normal'}];
 const c=await chat.loadChatGrounding(db,'u',today);
 const sealed=load('../execution/runningExecutionStore').sealRunningExecution('u',{sourceActivityId:'real-run',occurredAt:'2026-09-08',planSessionId:s.session_id,
  quantities:{totalDuration:{value:30,unit:'minutes'}},completeness:'FULL'},today);
 const modern=load('../execution/runningExecution').reconcileRunningExecutions([sealed.record]);
 c.facts.runningExecution=load('../execution/historicalRunning').mergeRunningHistory('u',db.tables.usuarios[0].workout_history,modern,today);
 const p=chat.projectChatGenerationFacts(c);
 assert.equal(p.RECENT_EXECUTED_TRAINING.length,1);
 assert.equal(p.RECENT_EXECUTED_TRAINING[0].provenance,'MODERN_STRUCTURED');
 assert.equal(p.RECENT_EXECUTED_TRAINING[0].quantities.totalDurationSeconds.value,1800);
 assert.equal(p.RECENT_EXECUTED_TRAINING[0].supportingEvidence.filter(e=>e.source==='completedSessions').length,1);
 assert.ok(p.HISTORICAL_CONTEXT.longitudinal.entries.filter(e=>['weekly_plan.sessions','usuarios.workout_history'].includes(e.source)).every(e=>e.executionRef===p.RECENT_EXECUTED_TRAINING[0].executionId));
 assert.equal(p.HISTORICAL_CONTEXT.legacyResponseEvidence[0].value,'normal');
});

test('RESPONSE links only recorded matching execution; legacy sensation and unmatched reports remain separate',async()=>{
 const db=unexecutedWeekFixture(),s=db.tables.weekly_plan[0].sessions[0];
 s.completada=true;
 s.chatExecutionEvidence=[{kind:'PERFORMED',source:'athlete_report',executionDate:'2026-09-08',quote:'Hice técnica',discipline:'box',responseQuotes:['Sin molestias inmediatas']},
  {kind:'RESPONSE',source:'athlete_report',executionDate:'2026-09-08',quote:'A las 24 horas sin molestias'},
  {kind:'RESPONSE',source:'athlete_report',executionDate:'2026-09-09',quote:'Otra ejecución'}];
 const p=chat.projectChatGenerationFacts(await chat.loadChatGrounding(db,'u',today));
 assert.equal(p.RESPONSE.length,2);assert.ok(p.RESPONSE.every(r=>r.executionRef&&r.date==='2026-09-08'));
 assert.equal(p.HISTORICAL_CONTEXT.unassociatedExecutionEvidence.length,1);
 assert.equal(p.HISTORICAL_CONTEXT.unassociatedExecutionEvidence[0].executionRef,null);
 assert.equal(p.HISTORICAL_CONTEXT.legacyResponseEvidence[0].value,'bien');
});

test('valid non-running legacy report stays unverified; unknown/future dates remain outside executed collection',async()=>{
 const db=unexecutedWeekFixture();db.tables.weekly_plan=[];
 db.tables.usuarios[0].workout_history=[{fecha:'2026-09-10',tipo:'natacion',duracion:40,notas:'Trabajo realizado'},
  {tipo:'box',notas:'Fecha desconocida'},{fecha:'2026-09-20',tipo:'box'}];
 const p=chat.projectChatGenerationFacts(await chat.loadChatGrounding(db,'u',today));
 assert.equal(p.RECENT_EXECUTED_TRAINING.length,1);assert.equal(p.RECENT_EXECUTED_TRAINING[0].discipline,'natacion');
 assert.equal(p.RECENT_EXECUTED_TRAINING[0].evidenceStatus,'LEGACY_REPORTED_UNVERIFIED');
 assert.equal(p.RECENT_EXECUTED_TRAINING[0].quantities,null);
 assert.ok(p.HISTORICAL_CONTEXT.longitudinal.entries.some(e=>e.value?.includes('Fecha desconocida')));
});

test('an explicit performed observation is not projected as absence of execution even with a stale completion flag',async()=>{
 const db=unexecutedWeekFixture();db.tables.usuarios[0].workout_history=[];
 db.tables.weekly_plan[0].sessions[0].chatExecutionEvidence=[{kind:'PERFORMED',source:'athlete_report',executionDate:'2026-09-08',discipline:'box',quote:'Hice técnica',responseQuotes:[]}];
 const p=chat.projectChatGenerationFacts(await chat.loadChatGrounding(db,'u',today));
 assert.equal(p.RECENT_EXECUTED_TRAINING.length,1);assert.equal(p.RECENT_EXECUTED_TRAINING[0].evidenceStatus,'ATHLETE_REPORTED_EXECUTION');
 assert.equal(p.RECENT_EXECUTED_TRAINING[0].quantities,null);assert.equal(p.PAST_PRESCRIPTIONS_WITHOUT_EXECUTION.length,3);
});

test('same-day plan and unlinked run do not establish identity or authorize deduplication',async()=>{
 const db=unexecutedWeekFixture();db.tables.weekly_plan[0].sessions[0].completada=true;
 db.tables.usuarios[0].workout_history[0].fecha='2026-09-08';
 const p=chat.projectChatGenerationFacts(await chat.loadChatGrounding(db,'u',today));
 assert.equal(p.RECENT_EXECUTED_TRAINING.length,2);
 assert.equal(p.RECENT_EXECUTED_TRAINING.filter(e=>e.provenance==='LEGACY_STRUCTURED').length,1);
 assert.equal(p.RECENT_EXECUTED_TRAINING.filter(e=>e.evidenceStatus==='COMPLETION_RECORDED').length,1);
});
