...
for (const reviewKind of ['recommendation', 'interpretation', 'legacy_unsupported']) {
 test('no-week longitudinal Coach: valid answer, one generation, zero mutations / ' + reviewKind, async () => {
  ...
  const result = await load('../chat/runChatCoach').runChatCoach(db, 'u', report, async (system, messages, observation) => {
   ...
   assert.ok(observedFacts.HISTORICAL_CONTEXT.longitudinal.entries.some(e => e.kind === 'USER_REPORTED_EVIDENCE' && e.value.includes('sin reacción posterior')));
  ...
  assert.equal(result.pipeline.relevantHistoryCount, observedFacts.HISTORICAL_CONTEXT.longitudinal.entries.length);
...
test('Generation temporal projection partitions current/future/history without changing authority facts', async () => {
 ...
 for(const key of ['runningExecution','longitudinal','coachingKnowledge']) assert.deepEqual(projected.HISTORICAL_CONTEXT[key],plain(context.facts[key]),key);
...
test('historical Tuesday/Thursday/Saturday and habitual days cannot populate CURRENT_WEEK',async()=>{
 ...
 assert.ok(p.HISTORICAL_CONTEXT.longitudinal.entries.some(e=>e.kind==='COACH_INTERPRETATION'&&e.date===null));
