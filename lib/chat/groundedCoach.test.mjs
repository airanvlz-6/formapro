...
for (const reviewKind of ['recommendation', 'interpretation', 'legacy_unsupported']) {
 test('no-week longitudinal Coach: valid answer, one generation, zero mutations / ' + reviewKind, async () => {
  ...
  const result = await load('../chat/runChatCoach').runChatCoach(db, 'u', report, async (system, messages, observation) => {
   ...
   assert.deepEqual(observedFacts.plan, []);
   ...
   assert.ok(observedFacts.longitudinal.entries.some(e => e.source === 'usuarios.workout_history' && e.value.includes('Trabajo técnico de fuerza')));
   assert.ok(observedFacts.longitudinal.entries.some(e => e.kind === 'USER_REPORTED_EVIDENCE' && e.value.includes('sin reacción posterior')));
  ...
  assert.equal(result.pipeline.relevantHistoryCount, observedFacts.longitudinal.entries.length);
