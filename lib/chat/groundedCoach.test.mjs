...
for (const reviewKind of ['recommendation', 'interpretation', 'legacy_unsupported']) {
 test('no-week longitudinal Coach: valid answer, one generation, zero mutations / ' + reviewKind, async () => {
  ...
  const result = await load('../chat/runChatCoach').runChatCoach(db, 'u', report, async (system, messages, observation) => {
   ...
   assert.ok(observedFacts.HISTORICAL_CONTEXT.longitudinal.entries.some(e => e.source === 'usuarios.workout_history' && e.value.includes('Trabajo técnico de fuerza')));
...
test('Generation temporal projection partitions current/future/history without changing authority facts', async () => {
 ...
 for(const key of ['completedSessions','prescriptions','exposure','recentFrequency','modifications']) assert.deepEqual(projected.HISTORICAL_CONTEXT[key],plain(context.facts.history[key]),key);
 for(const key of ['runningExecution','coachingKnowledge']) assert.deepEqual(projected.HISTORICAL_CONTEXT[key],plain(context.facts[key]),key);
...
test('historical Tuesday/Thursday/Saturday and habitual days cannot populate CURRENT_WEEK',async()=>{
 ...
 assert.deepEqual(plain(p.HISTORICAL_CONTEXT.prescriptions).map(s=>[s.day,s.date,s.source,s.temporal,s.factualState]),[
