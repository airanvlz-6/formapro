import { persistCoachingKnowledge } from './athleteCoachingKnowledge';
...
export async function runChatCoach(db: any, user: string, message: string, complete: ChatCompletion, today = chatToday()) {
  ...
  const knowledge = await persistCoachingKnowledge(db, user, message, today);
  ...
  const current = mutation.dates.length || knowledge.count ? await loadChatGrounding(db, user, today) : initial;
  const outcome = { mutation, adaptation, knowledge, supportedAutomaticChanges: ['temporary_unavailability_explicit_weekday'],
    unsupportedAutomaticChanges: ['temporary_equipment_capacity', 'clinical_restriction', 'medical_resolution', 'goal_or_event_change', 'execution_completion'],
    knowledgePolicy: 'Only athlete-supplied declarations are stored. Discomfort remains dated evidence, never a diagnosis or permanent clinical restriction.',
