...
test('D2A.1 legacy semantic catalog keeps exposure when dose is unitless',()=>{
  const r=merge([
    {fecha:'2026-08-09',tipo:'rodaje_largo_z2',duracion:103},
    {fecha:'2026-08-26',tipo:'series',duracion:62},
    {fecha:'2026-08-28',tipo:'rodaje_regenerativo',duracion:35},
    {fecha:'2026-08-31',tipo:'rodaje_z2',duracion:49},
  ]);
  assert.equal(r.records.length,4);
  assert.deepEqual(plain(r.records.map(x=>x.exposure)),['long_run','quality','recovery','easy']);
  assert.equal(r.summaries.all.exposureCounts.long_run,1);
  assert.equal(r.summaries.all.exposureCounts.quality,1);
  assert.equal(r.summaries.all.longRunExposure[0].date,'2026-08-09');
  assert.equal(r.summaries.all.qualityExposure[0].date,'2026-08-26');
  assert.equal(r.summaries.all.duration.value,null);
  assert.equal(r.records[0].metrics.totalDurationSeconds,undefined);
});
test('D2A.1 unknown legacy type remains conservatively rejected',()=>{
  assert.equal(merge([{fecha:'2026-09-01',tipo:'carrera inventada',duracion:40}]).records.length,0);
});
