...
test('P Q R: independent intensity resolution, policy/evidence/composition blockers; NOT_EVALUATED preserved',()=>{
  ...
  for(const b of ['DOSE_EVIDENCE_MISSING','RECENT_METHOD_EXECUTION_REQUIRED','DOSE_COMPOSITION_NOT_ESTABLISHED','NO_COMPATIBLE_EXECUTION']) assert.ok(entry.blockers.includes(b));
  ...
  assert.ok(available.blockers.includes('EXACT_METHOD_STRUCTURE_REQUIRED'));assert.ok(!available.blockers.includes('DOSE_EVIDENCE_MISSING'));
});
test('S: weekly deferred demand preserves simultaneous blockers before Planner',()=>{
  ...
  assert.equal(r.ok,false);assert.ok(r.deferredDoseDemands[0].blockers.includes('RECENT_METHOD_EXECUTION_REQUIRED'));
  assert.ok(r.deferredDoseDemands[0].blockers.includes('DOSE_EVIDENCE_MISSING'));
});
test('T: unsupported specific/economy policies and unsupported threshold interval composition stay unresolved',async()=>{
  for(const p of load('runningMethodDosePolicies').RUNNING_METHOD_DOSE_POLICIES.filter(p=>['running_specific','running_economy'].includes(p.methodId))) assert.equal(p.selectDose,null);
...
test('P: threshold reference can execute while required reuse evidence remains missing',()=>{
  ...
  assert.ok(e.blockers.includes('DOSE_EVIDENCE_MISSING'));assert.ok(e.blockers.includes('RECENT_METHOD_EXECUTION_REQUIRED'));
});
...
test('SQL grants append/read only to service and no client access; existing tables untouched',()=>{
  ...
  assert.equal(statements.filter(statement=>/^(begin|commit|rollback)\b/.test(statement)).length,2);
  const reset='revoke all privileges on table public.running_execution_records from public, anon, authenticated, service_role';
  const grant='grant select, insert on table public.running_execution_records to service_role';
  assert.deepEqual(statements.filter(statement=>/^revoke\b/.test(statement)),[reset]);
  // Exact grant list also excludes ordinary-role grants and UPDATE/DELETE/TRUNCATE authority.
  assert.deepEqual(statements.filter(statement=>/^grant\b/.test(statement)),[grant]);
  assert.ok(statements.indexOf(reset)<statements.indexOf(grant));
  assert.ok(statements.includes('alter table public.running_execution_records enable row level security'));
  assert.match(statements.join(';'),/primary key \(user_codigo, execution_id, content_digest\)/);
  assert.doesNotMatch(statements.join(';'),/\bif not exists\b/);
  assert.doesNotMatch(statements.join(';'),/\b(drop|truncate|delete|update|merge)\b|\binsert into\b/);
  assert.deepEqual(statements.filter(statement=>/^alter\b/.test(statement)),
