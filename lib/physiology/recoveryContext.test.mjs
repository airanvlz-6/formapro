...
test('HTTP boundary translates only recovery failures and rethrows unrelated errors', async () => {
  ...
  const legacySource = 'const coachFirstEnabled = () => false;\n' + source;
  for (const error of ['db_error', 'invalid_response', 'invalid_input']) {
    const r = await execute(legacySource, { RecoveryReadError, handlePost: async () => { throw new RecoveryReadError({ ok: false, error, reason: 'test' }); }, NextResponse: { json: (body, init) => ({ body, ...init }) } }, 'return POST({});');
    assert.equal(r.status, error === 'invalid_input' ? 400 : 503); assert.equal(r.body.error, error);
  }
  await assert.rejects(execute(legacySource, { RecoveryReadError, handlePost: async () => { throw Error('unrelated'); } }, 'return POST({});'), /unrelated/);
});
