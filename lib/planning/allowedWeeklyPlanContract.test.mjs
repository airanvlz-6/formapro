...
test('Focus preserves external activity and releases previous future REST decisions', async () => {
  ...
  assert.equal(p.fixedSessions.martes.tipo, 'external_blocked'); assert.equal(p.fixedSessions.domingo, undefined);
  ...
  assert.equal(conflict.ok, true); assert.equal(conflict.fixedSessions.martes.tipo, 'external_blocked');
});
...
test('infeasible protected RECOVERY stops before Planner instead of rebuilding or rewriting it', async () => {
  ...
  const result = await adapter.planBoundedWeek(fakeDatabase(tables()), 'test', {...request,
    preserveDays:['lunes'], snapshot:{sessions:[{dia:'lunes',tipo:'carrera',stimulusId:'recuperacion_activa'}]}}, async () => {calls++;return '{}';});
  assert.equal(result.code, 'WEEKLY_CONTRACT_UNSATISFIABLE'); assert.equal(calls, 0);
});
...
function strategicSelection(c) {
  if(c.contractVersion===2) return {contractVersion:2,contextDigest:c.contextDigest,selections:days.map(day=>({day,state:day==='jueves'?'TRAIN':'REST',
    decision:{role:'PRIMARY',reason:'Fixture: elección estructurada.'},...(day==='jueves'?{intent:{kind:'open_coach',version:1,discipline:'box',adaptationId:'fuerza_maxima',stimulusId:'fuerza_maxima',
      pattern:'horizontal_push',method:{kind:'coach_defined',label:'Controlled pushing'},role:'PRIMARY'}}:{})}))};
  ...
  function search(index, count, mask, selections) {
    ...
    for (const option of c.dayOptions[days[index]]) {
      ...
      const result = search(index + 1, nextCount, mask | bits(option), [...selections, { day: days[index], optionId: option.optionId,
        decision: { role: option.state === 'REST' ? 'RECOVERY' : 'PRIMARY', reason: 'Fixture: distribución permitida.' } }]);
...
async function actualRoute(overrides = {}, invalid = false, goal = 'max_strength') {
  ...
  // This test isolates Weekly proposal retries after longitudinal target admission.
  data.usuarios.ciclo_actual = { bloque: 'acumulacion', semana: 2, totalSemanas: 4, planningWeekStart: '2026-09-07', blockId: 'fixture-target' };
  ...
  const execute = vm.runInNewContext(compile(`async function execute() ${body}; execute;`), {
    datos:{weeklyContractVersion:2,generationToken:'test-token',targetWeekStart:'2026-09-07',empezarHoy:false,...overrides}, codigo:'test', supabase:db, apiKey:'test',
...
test('client cannot replace a canonical selection or persist the Analyzer prose as its objective', () => {
  ...
  assert.match(code,/if\(estructura.weeklyContractVersion!==2 && !resultadoIntegridad.valido && resultadoIntegridad.diasCorregir.length>0\) return null/);
});
