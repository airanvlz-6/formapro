...
import { sportsRuntime, contractFixture, plain, fakeDatabase, compile } from '../sports/trainingContractTestRuntime.mjs';
...
const tables = () => ({ usuarios: { modo_entrada: 'coach', categoria: 'box', perfil: { dias: 6 }, workout_history: [],
...
test('production server preparation reads bounded context and yields canonical v2-ready selection', async () => {
  ...
  assert.equal(db.calls.filter(t => t === 'athlete_coaching_notes').length, 1);
...
async function actualRoute(overrides = {}, invalid = false) {
  let calls = 0; const db = fakeDatabase(tables());
  const execute = vm.runInNewContext(compile(`async function execute() ${body}; execute;`), {
    datos:{weeklyContractVersion:1,targetWeekStart:'2026-09-07',empezarHoy:false,...overrides}, codigo:'test', supabase:db, apiKey:'test',
    ...
    NextResponse:{json:v=>v}, fetch:async (_url, init)=>{calls++;const c=extractContract(JSON.parse(init.body).messages[0].content);
      return {ok:true,json:async()=>({content:[{text:invalid?'{}':JSON.stringify(selection(c))}]})};},
  });
  return {result:await execute(),calls,db};
}
test('actual route bounds retries server-side and still issues the unchanged calendar receipt', async () => {
...
test('server output preserves completed data and protected pending recovery exactly', async () => {
  ...
  const result=await adapter.planBoundedWeek(fakeDatabase(data),'test',{...request,snapshot:{sessions:[old,completed]}},async prompt=>{
    const c=extractContract(prompt);return JSON.stringify(selection(c));
  });
...
test('Builder receives and independently checks canonical state and intent v2 without weekly cryptographic binding', async () => {
  ...
  },async()=>JSON.stringify({stimulusId:'fuerza_maxima',structureId:'strength_sets',blocks:['warmup','main','cooldown'].map(blockType=>({
    blockType,movements:[{movementId:'bench_press',prescription:{reps:5}}],
  }))}));
  assert.equal(good.ok,true,JSON.stringify(good));assert.equal(good.trainingContract.contractVersion,2);
});
