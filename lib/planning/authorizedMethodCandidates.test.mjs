...
for(const extra of [{applicableStrategies:['10k']},{applicablePhases:['realization']}])test(`T4: strategy/phase exclusion ${JSON.stringify(extra)}`,()=>{
 const f=setup({relations:[relation(extra)]});assert.equal(f.build().ok,true);assert.ok(!f.calls.some(c=>c.intent.transfer));
...
test('T7: unknown compatibility stays excluded in the transferred method',()=>{
 ...
 const r=f.build();assert.equal(r.ok,true);assert.equal(transferred(r).length,0);assert.ok(f.calls.some(c=>c.intent.transfer));
});
...
for(const kind of ['EQUIVALENT','MAINTENANCE','PARTIAL'])test(`T11: ${kind} coverage is explicit and only equivalent satisfies full coverage`,()=>{
 ...
 assert.equal(coverage.statuses.includes('TRANSFER_EQUIVALENT'),kind==='EQUIVALENT');
 if(!r.contract.strategy.coverage.some(c=>c.adaptationId==='tecnica'))assert.ok(coverage.statuses.includes('DEFERRED'));
...
test('T14: empty transfer catalog retains direct goal alternatives during deload',()=>{
 ...
 const ids=[...new Set(Object.values(r.contract.dayOptions).flat().flatMap(o=>o.intent?[o.intent.methodId]:[]))];
 for(const id of ['box_technique','running_recovery','box_max_strength'])assert.ok(ids.includes(id));
 assert.equal(transferred(r).length,0);
});
  ...
  {targetWeekStart:'2026-09-07',today:'2026-09-06',empezarHoy:false,snapshot:null,strategyVersion:1},async prompt=>{
   const c=JSON.parse(prompt.split('WEEKLY_CONTRACT:\n')[1]);return JSON.stringify({contractVersion:1,contextDigest:c.contextDigest,
    selections:Object.entries(c.dayOptions).map(([day,options])=>({day,optionId:(options.find(o=>o.intent?.transfer)||options[0]).optionId,
      decision:{role:'SUPPORTING',reason:'Fixture: transferencia autorizada para el día.'}}))});
  },'fixture-token');
