...
test('FORGE12 corrected week: only easy is dose-capable under the stated numeric hypothesis',async(t)=>{
  ...
  assert.deepEqual(plain(entries.filter(e=>e.prescriptionAllowed).map(e=>e.methodId)),['running_base']);
  assert.deepEqual(plain(entries.filter(e=>e.doseCapability==='QUANTIFIABLE').map(e=>e.methodId)),['running_base']);
  ...
  assert.equal(base.longitudinalDose.selectedDose.maximumAuthorized,3000);
  assert.equal(base.fixedPrescription.quantity,3000);
  assert.deepEqual(plain(base.fixedPrescription.intensity),{kind:'rpe',value:2,max:3});
  for(const day of ['viernes','sabado'])assert.deepEqual([...new Set(w.dayOptions[day].filter(o=>o.intent).map(o=>o.intent.methodId))],['running_base']);
  assert.ok(w.strategy.deferred.some(d=>d.reference==='adaptation:umbral'));
...
test('FORGE12 selection rejects the fixed pair before Builder, retry uses authorized REST and passes unchanged whole-week validation',async()=>{
...
test('FORGE12 Wednesday and Thursday remain unexecuted; planned coverage is not executed evidence',async()=>{
  ...
  // Existing canonical active-week projection marks elapsed unreported days unavailable.
  assert.equal(ctx.input.fixed.miercoles.state,'UNAVAILABLE');assert.equal(ctx.input.fixed.jueves.state,'UNAVAILABLE');
