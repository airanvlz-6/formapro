...
import { runningDoseTestRuntime, testDoseProposal } from '../sports/runningDoseV2TestFixture.mjs';
...
test('canonical squat weakness informs strength without excluding other feasible patterns', () => {
  const c = built({ weakness: true }), strength = Object.values(c.dayOptions).flat().filter(o => o.intent?.adaptationId === 'fuerza_general');
  assert.ok(strength.some(o => o.intent.pattern === 'squat' && o.intent.weaknessId === 'squat-weakness'));
  assert.ok(strength.some(o => o.intent.pattern !== 'squat' && o.intent.weaknessId === null));
...
test('restriction eliminates weakness methods before coverage; no unsafe replacement', () => {
  ...
  assert.ok(!Object.values(rejected.contract.dayOptions).flat().some(o => o.intent?.pattern === 'squat'));
...
test('legal IDs admit an alternative week and report uncovered recommendations', () => {
  const c = built(), p = choose(c);
  for (const s of p.selections) if (s.day !== 'lunes') s.optionId = c.dayOptions[s.day][0].optionId;
  const admitted = api.validateWeeklySelection(c, p);
  assert.equal(admitted.ok, true);
  assert.ok(admitted.warnings.some(w => w.code === 'WEEKLY_STRATEGY_COVERAGE_ADVISORY'));
});
...
test('real 3A -> strategy -> Planner -> signed Builder -> admitted persisted sessions and objective', async () => {
  // Isolated TEST policies preserve end-to-end signing/persistence coverage while production v2 is unresolved.
  const load = runningDoseTestRuntime();
  const db = strategyDatabase(), weekly = load('../planning/weeklyCalendarAuthority'), sessions = load('sessionAuthority');
  // Declaration remains context, not quantitative authorization.
  db.tables.usuarios.perfil.km_semana = 30;
  ...
  for (const slot of p.sessions) {
    ...
    const b = await sessions.generateTrainingSession(db, 'fixture', { targetWeekStart: '2026-09-07', day: slot.dia,
      ...
      if (c.runningMethodDose?.version === 2) return JSON.stringify(testDoseProposal(load, c));
      const libs = load('workoutStructureLibrary').WORKOUT_STRUCTURE_LIBRARY, semantics = load('structureSemantics');
      const dose = c.runningMethodDose?.dose;
      const structureId = c.allowedStructureIds.find(id => (!dose || dose.structures.includes(id)) && semantics.isStructureSatisfiable(libs[id], c.allowedMovementIds));
      ...
      if (dose) {
        const intervals = structureId === 'intervalos_carrera' || dose.metric === 'repetitions';
        const sets = intervals ? dose.interval.minimumSets : 1;
        for (const block of proposal.blocks) for (const movement of block.movements) {
          const intensity = movement.prescription.intensity;
          movement.prescription = block.blockType === 'main' ? {
            [dose.unit === 'meters' ? 'distanceMeters' : 'durationSeconds']: dose.boutSeconds?.minimum ?? dose.maximumAuthorized / sets,
            ...(intervals ? { sets, restSeconds: dose.interval.minimumRestSeconds } : {}), intensity,
          } : { durationSeconds: 60, intensity };
        }
      }
