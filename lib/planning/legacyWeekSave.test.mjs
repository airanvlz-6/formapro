...
async function backend(options = {}) {
  ...
  const execute = executable(actionBody, {
    ...
    getCanonicalRestrictions: async () => {
      if (options.restrictionsError) throw new Error('RESTRICTIONS_NOTES_READ_FAILED');
      return { restrictions: [] };
    },
...
test('restriction read failure stops week save before gate or any write', async () => {
  const result = await backend({ restrictionsError: true });
  assert.equal(result.response.status, 503);
  assert.equal(result.response.body.error, 'RESTRICTIONS_NOTES_READ_FAILED');
  assert.equal(result.validationInput, undefined);
