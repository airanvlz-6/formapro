...
test('missing structure mapping and missing suitable_for never veto concrete open design', () => {
  ...
  assert.equal(validate(load, c, p).ok, true, 'unknown format remains executable coaching');
});
...
test('missing 1RM remains symbolic with no fabricated kg; RPE stays valid', () => {
  ...
  assert.equal(load('structuredSession').renderContractSession(c,p).structuredPrescription.calculatedLoads.length,0);
  assert.equal(validate(load, c).ok, true);
});
test('known incompatibility rejects; unknown biomechanics is not a veto', () => {
...
test('generated movement outside literal catalog resolves and remains reference-incompatible', () => {
  ...
  assert.equal(validate(load, c, p).ok, true);
  assert.equal(load('movementVariants').resolvedMovement(entry),undefined);
});
