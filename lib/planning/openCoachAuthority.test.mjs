...
const runtime = () => sportsRuntime({ Date: FixedDate, console: { log() {}, info() {}, warn() {}, error() {} } });
...
for (const state of ['unavailable', 'unknown']) test(`required equipment ${state} retains its epistemic rejection`, () => {
  ...
  assert.ok(result.violations.some(e => e.startsWith(state === 'unavailable' ? 'FACTUAL_REQUIREMENT_UNAVAILABLE:' : 'REQUIREMENT_UNKNOWN:')), JSON.stringify(result));
});
...
test('real Weekly -> Session v4 -> signed save chain retains sibling context and freshness', async () => {
  const load = runtime(), { db, tables, days } = database(load);
...
test('open design still rejects hard time overflow and main-pattern mismatch', () => {
  ...
  assert.ok(validate(load, c, proposal(c, 'air_squat')).violations.includes('INTENT_NOT_SATISFIED'));
});
