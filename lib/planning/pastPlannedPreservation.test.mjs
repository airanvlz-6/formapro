...
test('FORGE12 snapshot → fixed calendar → signed survivors → assembly → real CAS payload preserves complete planned-only Wednesday',async()=>{
  ...
  const planned=await prep.planBoundedWeek(f.db,'u',f.request,async prompt=>{
    ...
    const coaching=JSON.parse(prompt.split('COACHING_CONTEXT:\n')[1].split('\nWEEKLY_CONTRACT:')[0]);
    const wed=coaching.past.prescriptionHistory.items.find(s=>s.date==='2026-09-09');
    assert.equal(wed.factualState,'PLANNED_ONLY');assert.equal(wed.execution,null);
    assert.equal(wed.prescription.durationMinutes,87);assert.equal(wed.prescription.methodId,'running_specific');
    assert.equal(coaching.current.readiness.status,'unknown');
    return JSON.stringify({contractVersion:1,contextDigest:c.contextDigest,selections:calendar.calendarDays.map(day=>{
      ...
      assert.ok(o,day);return {day,optionId:o.optionId,decision:{role:o.state==='REST'?'RECOVERY':'PRIMARY',reason:'Fixture: conservar historia sin inferir ejecución.'}};
    })});
  },generation.token);
  ...
  assert.deepEqual(plain(after.input.doseCapabilities.entries.filter(e=>e.prescriptionAllowed).map(e=>e.methodId)),['running_base','running_recovery','running_long_run','running_threshold']);
