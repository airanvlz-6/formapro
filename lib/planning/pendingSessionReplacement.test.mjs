...
function fixture() {
  return { user_codigo: 'audit', week_start: week, confidence: 80, updated_at: 'original',
...
test('full candidate preserves week metadata, order, other sessions and exact execution state', async () => {
  for (const completion of [null, undefined, false]) {
    ...
    assert.deepEqual(r.writes[0].payload, { sessions: expected.sessions });
...
test('replacement command/context/changeSet are explicit; only typed prescription from pending', async () => {
  const r = await run();
  assert.deepEqual(plain(r.input.command), { operationType: 'replace_session', source: 'pending_confirmation',
...
test('pending resolution uses exact ID/user/still-pending, checks row and returns partial on failure', async () => {
  ...
  const r = await run();
  assert.deepEqual(r.response.body, { ok: true, ejecutado: true, tipo: 'modificar_sesion', warnings: [] });
});
