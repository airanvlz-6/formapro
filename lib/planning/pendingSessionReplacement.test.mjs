...
async function run(options = {}) {
  ...
  const execute = vm.runInNewContext(`${executable}\nexecute`, {
    ...
    // Persistence fixtures assume sports approval; sports tests exercise the real rejection gate.
    verifySessionReceipt: () => {}, assertCalendarMutation: async () => {}, assertFreshSessionRestrictions: async () => {}, assertCurrentPrescriptionScope: async () => {},
...
test('restoration returns the real ID and frontend retains it without placeholder', () => {
  ...
  // 2E.2 closes the prose extractor; successful structured routes still retain real IDs.
  assert.match(branchFor('detectar_propuesta_sesion').getText(route), /ok: false.*STRUCTURED_SESSION_REQUIRED/);
});
