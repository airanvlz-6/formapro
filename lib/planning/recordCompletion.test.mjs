...
const fixedTime = '2026-09-06T23:30:00.000Z'; // Monday in Canary, Sunday in UTC.
class AuditDate extends Date { constructor(...args) { super(...(args.length ? args : [fixedTime])); } }
const load = planningTestRuntime({ Date: AuditDate });
...
const planFixture = () => withPlanIdentity({ user_codigo: 'audit', week_start: '2026-09-07', block_name: 'Base',
...
function harness(options = {}) {
    ...
    const q = {
      ...
      then(resolve, reject) { return Promise.resolve().then(() => {
        ...
        if (table === 'weekly_plan') data = op === 'read' ? plan : {
          id: plan.id, revision: payload.revision,
  ...
  const adapter = async (db, input) => {
    ...
    return recordPlanCompletion(db, input, async value => {
      ...
      return validateTestCandidate(validatePlanMutation, value, options.returnedCandidate);
    });
  };
...
async function runRoute(action, options = {}) {
  ...
  const execute = vm.runInNewContext(compile(`${emitter.getText(route)}\nasync function execute() ${branchFor(action).getText(route)}\nexecute;`), {
    ...
    ...sportsRuntime()('../execution/reportExecutionDate'), supabase: h.supabase, recordPlanCompletion: h.adapter, resolveCompletionDate, Date: AuditDate,
...
test('first completion keeps full plan/prescription/internal fields and gates before write', async () => {
  ...
  assert.deepEqual(plain(r.inputs[0].command), { operationType: 'record_completion', source: 'explicit_completion', expectedRevision: 5,
...
test('W6 enriches with controlled fields, retaining workout_id/duration/analysis/other metadata', async () => {
  ...
  assert.equal(r.response.body.historyRecorded, true); assert.equal(r.adapterCalls[0].fecha, '2026-09-07');
});
...
for (const action of ['marcar_sesion_completada', 'verificar_sesion_completada_deterministico']) {
  for (const [option, status, code] of [['noRowAt', 'conflict', 'PLAN_REVISION_CONFLICT'], ['errorAt', 'error', 'PLAN_PERSISTENCE_ERROR'], ['throwAt', 'unknown', 'PLAN_PERSISTENCE_UNKNOWN']]) {
    test(`B1 ${action} ${status}: one CAS and no dependent effects`, async () => {
      const r = await runRoute(action, { [option]: 'weekly_plan:update' });
      ...
      assert.equal(r.response.body.retryable, false); assert.equal(planWrites(r).length, 1);
      assert.equal(r.events.at(-1), 'weekly_plan:update');
      ...
      assert.deepEqual(planWrites(r)[0].payload.sessions.map(s => s.session_id), planFixture().sessions.map(s => s.session_id));
      if (action === 'verificar_sesion_completada_deterministico') assert.equal(r.response.body.historyRecorded, true);
    });
  }
}

test('future completion write is rejected before any database access; Canary today remains admissible',async()=>{
 const future=await runAdapter({evidence:{fecha:'2026-09-09'}});
 assert.equal(future.result.error,'FUTURE_COMPLETION_NOT_ALLOWED');assert.equal(future.queries.length,0);assert.equal(future.writes.length,0);
 const current=await runAdapter({evidence:{fecha:'2026-09-07'}});assert.equal(current.result.planCompleted,true);
});
