...
function execute(code, extra) {
  const context = { ...api, coachFirstPlanning: undefined, ...extra };
...
test('actual Planner HTTP normalization ignores submitted raw digest and forwards only valid cookie evidence', async () => {
  ...
  for (const cookie of [null, `${api.ENVIRONMENT_CONFIRMATION_COOKIE}=${freshToken}`]) {
    const datos = { targetWeekStart: binding.weekStart, generationToken: binding.generationToken, confirmedAvailabilityDigest: digest, weeklyContractVersion: 2, empezarHoy: false };
    let received;
