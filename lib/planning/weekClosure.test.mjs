...
const optionalHelpers = ['canonicalPhysiologyForAnalytics', 'latestCanonicalHrv', 'detectarCelebraciones', 'ejecutarDiscoveryEngine', 'ejecutarAthleteResponseEngine']
  .map(name => findAll(route, n => ts.isFunctionDeclaration(n) && n.name?.text === name)[0].getText(route)).join('\n');
const load = planningTestRuntime();
...
const { mutatePlanWithCAS, planPersistenceFailure } = load('planPersistence');
const plain = v => JSON.parse(JSON.stringify(v));
const week = '2026-08-31';
const time = '2026-09-05T23:30:00.000Z'; // Sunday Canary, still Saturday UTC.
function fixture() {
  return withPlanIdentity({ user_codigo: 'audit', week_start: week, resumen_semana: 'Old', updated_at: 'before',
...
async function run(action = 'close_week', options = {}) {
  ...
  const supabase = { from(table) {
    ...
    const query = {
      ...
      then(resolve, reject) { return Promise.resolve().then(() => {
        ...
        if ([key, specific].includes(options.errorAt)) return { data: null, error: { code: '23514', message: key }, status: 400 };
        if ([key, specific].includes(options.noRowAt)) return { data: null, error: null };
        let data;
        if (op !== 'read') data = { id: table === 'weekly_plan' ? plan.id : 'written', codigo: 'audit', user_codigo: 'audit', week_start: week, ...payload,
  ...
  const execute = vm.runInNewContext(compile(`${optionalHelpers}\nasync function execute() ${branch(action).getText(route)}\nexecute;`), {
    ...
    validatePlanMutation: async input => {
      ...
      return validateTestCandidate(validatePlanMutation, input, options.returnedCandidate);
    },
...
test('W7 projects the complete plan, changes only a past rest flag, and gates before all writes', async () => {
  ...
  assert.deepEqual(plain(r.inputs[0].command), { operationType: 'complete_past_rest_days', source: 'week_close', expectedRevision: 5,
  ...
  assert.deepEqual(plain(planWrites(r)[0].payload), { sessions: expected.sessions, revision: 6 });
...
test('W8 gates a complete candidate changing only summary; Coach adherence is not a canonical metric', async () => {
  ...
  assert.deepEqual(plain(planWrites(r)[0].payload), { resumen_semana: 'New summary', revision: 6 });
...
test('real close button advances only on explicit complete closure, never partial or closed:false', async () => {
  ...
  for (const result of [{ ok: false, partial: true, closed: false, planPersisted: true }, { ok: true, closed: false },
    ...
    const click = vm.runInNewContext(compile(`const click = ${handler.getText(web)};\nclick;`), {
      ...
      availabilityQuestion: async () => 'Actualmente tengo tu disponibilidad así. ¿Sigue siendo correcta?',
      setMostrarBotonNuevaSemana() {}, cargarPlanSemanal() {}, setMensajes() {},
      setEsperandoConfirmacionDisponibilidad: v => { advance = v; },
      dispararGeneracion: async () => { advance = true; },
    });
...
for (const action of ['close_week', 'guardar_resumen_semana']) {
  for (const [option, status, code] of [['noRowAt', 'conflict', 'PLAN_REVISION_CONFLICT'], ['errorAt', 'error', 'PLAN_PERSISTENCE_ERROR'], ['throwAt', 'unknown', 'PLAN_PERSISTENCE_UNKNOWN']]) {
    test(`B1 ${action}: ${status} stops dependent effects without replay`, async () => {
      const r = await run(action, { [option]: 'weekly_plan:update' });
      assert.equal(r.response.status, 200); assert.equal(r.response.body.ok, false);
      assert.equal(r.response.body.error, code); assert.equal(r.response.body.persistenceStatus, status);
      assert.equal(r.response.body.retryable, false); assert.equal(r.response.body.commitConfirmed, false);
      assert.equal(r.writes.length, 1); assert.equal(planWrites(r).length, 1);
      assert.deepEqual(planWrites(r)[0].filters, { id: 'plan-1', user_codigo: 'audit', revision: 5 });
      assert.equal(planWrites(r)[0].payload.revision, 6); assert.equal(r.prompts.length, 0);
      assert.equal(r.queries.filter(q => q.table === 'weekly_plan' && q.op === 'read').length, 1);
    });
  }
  test(`B1 ${action}: identity and single revision increment`, async () => {
    const r = await run(action);
    assert.equal(r.response.body.ok, true);
    assert.equal(planWrites(r)[0].payload.revision, 6);
    assert.deepEqual(r.inputs[0].candidate.sessions.map(s => s.session_id), fixture().sessions.map(s => s.session_id));
    assert.equal(r.inputs[0].candidate.revision, 5);
    if (action === 'guardar_resumen_semana') assert.equal(Object.hasOwn(planWrites(r)[0].payload, 'sessions'), false);
  });
}
