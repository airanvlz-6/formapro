...
test('actual save route verifies real weekly/session chain, admits identity and INSERT revision 1; replay fails without another write',async()=>{
  ...
  assert.equal(saved.commitConfirmed,true);assert.equal(saved.persistenceStatus,'committed');assert.deepEqual(plain(saved.persistenceReceipt),{planId:'saved-plan',revision:1,weekStart:week});
...
for(const repairWorks of [false,true])test(`real parallel-candidate batch gate: repair success=${repairWorks}, invalid week never writes`,async()=>{
  ...
  else { assert.equal(saved.code,'WEEK_REPAIR_FAILED');assert.equal(saved.saveStage,'whole_week_repair');
    assert.equal(saved.persistenceStatus,'not_committed');assert.equal(saved.commitConfirmed,false);
    assert.ok(saved.repairDiagnostics?.length); }
});
...
test('RECOVERY v2 round trip through actual weekly save and context reload preserves canonical identity',async()=>{
  ...
  const reloaded=await planner.prepareAllowedWeeklyPlanContract(db,user,{...request,snapshot:db.row,preserveDays:['lunes']});
...
test('valid main_pattern RECOVERY v2 round trips structured intent without inferring from prose',async()=>{
  ...
  const reloaded=await planner.prepareAllowedWeeklyPlanContract(db,user,{...request,snapshot:{sessions:[persisted]},preserveDays:['lunes']});
...
for(const intent of [null,'stimulus_only',[],{kind:'main_pattern',pattern:'unknown'},{kind:'stimulus_only',pattern:'run'}])
  test(`malformed persisted RECOVERY intent ${JSON.stringify(intent)} remains fail-closed`,async()=>{
    ...
    const result=await planner.prepareAllowedWeeklyPlanContract(db,user,{...request,snapshot:{sessions:[persisted]},preserveDays:['lunes']});
...
test('legacy/corrupt protected RECOVERY without stimulus metadata remains unresolved despite its title',async()=>{
  ...
  for(const extra of [{},{intent:{kind:'stimulus_only'}}]) {
    let calls=0;
    const result=await planner.planBoundedWeek(db,user,{...request,preserveDays:['lunes'],snapshot:{sessions:[{
...
test('R2 R3 R6: active regeneration preserves signed history, rebuilds future slots and admits Builder evidence',async()=>{
 const db=database();
 const completed={dia:'martes',tipo:'box',completada:true,titulo:'Historical',descripcion:'Unchanged'};
 const oldRest={dia:'viernes',tipo:'descanso',titulo:'Old rest',descripcion:'Replaceable'};
 db.row={id:'old-plan',user_codigo:user,week_start:week,revision:3,sessions:[completed,oldRest]};
 const before=JSON.stringify(db.row);
 const p=await plan(db,undefined,{today:'2026-09-09',empezarHoy:true,snapshot:db.row});
 const admission=weekly.weeklySaveAdmission(p.calendarReceipt,user,week,db.row.sessions);
 assert.equal(admission.outcome,null);assert.deepEqual(plain(admission.survivorIndices),[0]);
 const {result}=await build(db,p);assert.equal(result.ok,true,JSON.stringify(result));
 const assembled=p.sessions.map(s=>s.dia==='martes'?completed:s.dia==='jueves'?result.sesion:s);
 await weekly.assertWeeklyCalendar(db,user,week,assembled,p.calendarReceipt,{requireV2:true,generationToken:token});
 assert.equal(JSON.stringify(db.row),before);
});
test('R1 R7: signed empty regeneration cannot admit a save, even when calendar has seven slots',()=>{
 const slots=days.map(day=>({day,state:day==='martes'?'TRAIN':'REST',protected:true}));
 for(const pendingManagedDays of [[],['jueves']]) {
  const receipt=signed({codigo:user,week,expires:Date.now()+10000,protocolVersion:2,admittedSlots:slots,regeneration:{pendingManagedDays}});
  const r=weekly.weeklySaveAdmission(receipt,user,week,[]);
  assert.equal(r.outcome.canContinue,false);
  assert.equal(r.outcome.code,pendingManagedDays.length?'NO_NEW_EXECUTABLE_PRESCRIPTION':'WEEKLY_REGENERATION_NO_OP');
 }
});
