...
test('production legacy scalar Sunday resolves before strict calendar validation',async()=>{
 const p={...profile,distribucion_semanal:{...distribution,carrera_larga:'domingo'}};
 const database=fakeDatabase({usuarios:p,athlete_training_sources:[],weekly_plan:[]});
 const sessions=week('domingo','carrera');
 const receipt=await calendar.issueWeeklyCalendar(database,'u','2026-09-07',sessions);
 await calendar.assertWeeklyCalendar(database,'u','2026-09-07',sessions,receipt);
});
