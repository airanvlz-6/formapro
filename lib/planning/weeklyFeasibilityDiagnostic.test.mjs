...
for (const temporalIntent of [undefined, true, false]) test(`preflight preserves run ID and temporal intent ${temporalIntent} without a catalog veto before the open proposal`, async () => {
  events.length = 0;
  ...
  assert.equal(r.ok,true);assert.equal(r.canContinue,true);
  assert.equal(r.availabilityStatus,'VALID');assert.equal(r.temporalStatus,'TEMPORAL_DECISION_RESOLVED');
  assert.deepEqual(plain(r.temporalDecision),{includeToday:temporalIntent,reason:'explicit_intent'});
