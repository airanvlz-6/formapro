...
test('successful diagnostics do not affect contract or context digest and are opt-in', () => {
  events.length = 0;
  ...
  assert.equal(result.contract.contextDigest, createHash('sha256').update(JSON.stringify(input)).digest('hex'));
  const enabled=api.buildAllowedWeeklyPlanContract(input,{...metadata,includeFeasible:true});
  assert.equal(events.filter(e=>e[0]==='WEEKLY_FEASIBILITY_REJECTION_DIAGNOSTIC').length,1);assert.deepEqual(plain(result),plain(enabled));
...
for (const temporalIntent of [undefined, true, false]) test(`preflight forwards run ID and actual temporal intent ${temporalIntent} returns explicit no-prescription for active regeneration`, async () => {
  events.length = 0;
  ...
  const quiet = sportsRuntime({ console: { log() {}, warn() {}, info: (...args) => events.push(args) } }, numericPolicyFixture);
  ...
  if(temporalIntent===undefined){assert.equal(r.code,'TEMPORAL_DECISION_REQUIRED');assert.equal(r.canContinue,false);assert.equal(JSON.stringify(tables),before);return;}
  assert.deepEqual(plain(r), { ok: false, code: 'NO_NEW_EXECUTABLE_PRESCRIPTION', reason:'NO_FEASIBLE_REMAINING_SELECTION',noOp:false,
    availabilityStatus: 'VALID', canContinue: false, temporalStatus:'TEMPORAL_DECISION_RESOLVED',
    temporalDecision: {includeToday:temporalIntent,reason:'explicit_intent'} });
  const logs = events.filter(e => e[0] === 'WEEKLY_FEASIBILITY_REJECTION_DIAGNOSTIC');
  assert.equal(logs.length, 0, 'the legacy aggregate rejection is not emitted for this distinct outcome');
