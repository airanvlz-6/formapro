...
test('today unavailable still asks when future managed days remain', async () => {
  const r = await run(database(), { today: '2026-09-11' }); assert.equal(r.canContinue, false);
  assert.equal(r.code, 'TEMPORAL_DECISION_REQUIRED');
});
for (const session of [{ dia: 'lunes', tipo: 'carrera', completada: true }, { dia: 'lunes', tipo: 'descanso' }, { dia: 'lunes', tipo: 'sin_registrar' }]) {
  test(`today ${JSON.stringify(session)} asks while remaining interval is relevant`, async () => {
    const r = await run(database(), { snapshot: { sessions: [session] } });
    assert.equal(r.canContinue, false, JSON.stringify(r));assert.equal(r.preflightRequirement.kind,'temporal');
  });
}
...
test('temporal choice is independent of the weekly count budget and reads no feasibility data',async()=>{
 const db=database();const r=await run(db);
 assert.equal(r.code,'TEMPORAL_DECISION_REQUIRED');
 assert.ok(!db.calls.includes('athlete_coaching_notes'));assert.ok(!db.calls.includes('athlete_state_events'));
});
...
test('resolved goal never prompts again; conflicting authority still blocks', async () => {
  ...
  const conflict = await run(db, { temporalIntent: false }); assert.equal(conflict.canContinue, false); assert.equal(conflict.goalRequirement.resolution.status, 'GOAL_CONFLICT');
});
...
for (const [name, body] of [['chat', chat.thenStatement.getText(ast)], ['button', button.body.getText(ast)]]) {
  test(`${name} delegates to the shared weekly confirmation entry without duplicating its question`, async () => {
    let calls = 0;
...
test('real orchestrator calls shared preflight before Analyzer and stops on an unresolved temporal choice', async () => {
  ...
  const execute = vm.runInNewContext(compile(`const run=${declaration.initializer.getText(ast)};run;`), {
    codigoUsuario: 'fixture', weeklyPlanningContinuationRef:{current:null},availabilityConfirmationRef:{current:null},console: { log() {} }, apiCall: async body => {
...
test('availability clarification preserves the original temporal request for the shared preflight', async () => {
  ...
  const run = vm.runInNewContext(compile(`async function run() ${branch.thenStatement.getText(ast)}; run;`), {
    codigoUsuario: 'fixture', texto: 'carrera lunes y box martes', weeklyPlanningContinuationRef:{current:null}, availabilityConfirmationRef: { current: null },
...
function productionDatabase() {
  const db = database({ especialidad: 'funcional_crossfit', categoria: 'funcional',
    objetivo_principal: { descripcion: 'Open CrossFit Games 2027 - estándares Masters' },
    distribucion_semanal: { box: ['martes', 'jueves', 'viernes', 'sabado'], carrera: ['lunes', 'miercoles', 'domingo'] } });
  db.tables.athlete_training_sources.forEach(s => { s.dias = db.tables.usuarios.distribucion_semanal[s.disciplina]; });
  return db;
}
const availabilityApi = load('chatAvailability');
for (const reply of ['sí', 'sí, es correcta', 'sigue siendo así', 'sigue igual', 'todo correcto']) {
  test(`production: ${reply} -> temporal choice -> CrossFit admission, no goal question or writes`, async () => {
    const db = productionDatabase(), before = JSON.stringify(db.tables);
    const question = await availabilityApi.readAvailabilityConfirmation(db, 'fixture');
    assert.equal(question.ok, true); assert.match(question.question, /correcta para esta semana/);
    const accepted = await availabilityApi.updateChatAvailability(db, 'fixture', reply, question.snapshotDigest);
    assert.equal(accepted.ok, true); assert.equal(accepted.actualizado, false);
    const choice = await run(db);
    assert.equal(choice.preflightRequirement.kind, 'temporal'); assert.equal(choice.goalRequirement, undefined);
    const result = await run(db, { temporalIntent: 'Próximo día disponible', temporalReply: true });
    assert.equal(result.canContinue, true); assert.equal(result.temporalDecision.includeToday, false);
    assert.equal(result.goalRequirement, undefined); assert.doesNotMatch(JSON.stringify(result), /GOAL_UNSUPPORTED|STRATEGY_UNSUPPORTED/);
    const prepared = await load('../planning/prepareAllowedWeeklyPlanContract').prepareAllowedWeeklyPlanContract(db, 'fixture',
      { ...request, empezarHoy: false, strategyVersion: 1 });
    assert.equal(prepared.ok, true); assert.equal(prepared.contract.strategy.goal.id, 'crossfit');
    assert.ok(prepared.contract.strategy.methods.includes('running_base'));
    const context = await load('../athlete/loadAthletePrescriptionContext').loadAthletePrescriptionContext(db, 'fixture', { asOfDate: request.today });
    assert.equal(context.planningStrategy.strategyId, 'crossfit');
    assert.equal(context.goals.primary.resolved.value, db.tables.usuarios.objetivo_principal.descripcion);
    await assert.rejects(() => load('../athlete/goalAnswers').requireGoalAuthority(db, 'fixture'), /CHANGED_RETRY/);
    assert.equal(JSON.stringify(db.tables), before);
  });
}
test('negative availability reply stays in correction without mutation or advancing', async () => {
  const db = productionDatabase(), before = JSON.stringify(db.tables);
  const q = await availabilityApi.readAvailabilityConfirmation(db, 'fixture');
  const result = await availabilityApi.updateChatAvailability(db, 'fixture', 'no', q.snapshotDigest);
  assert.equal(result.ok, false); assert.equal(result.responseKind, 'UNRESOLVED_AVAILABILITY_RESPONSE');
  assert.equal(JSON.stringify(db.tables), before);
});
for (const [name, body] of [['chat', chat.thenStatement.getText(ast)], ['button', button.body.getText(ast)]]) {
  test(`real ${name} entry shows the weekly availability checkpoint before orchestration`, async () => {
    const db = productionDatabase(), messages = [], actions = [], ref = { current: null };
    let waiting = false;
    const entry = find(ast, n => ts.isVariableDeclaration(n) && n.name.getText(ast) === 'dispararGeneracion').initializer;
    const apiCall = async body => {
      actions.push(body.action);
      if (body.action === 'close_week') return { ok: true, closed: true };
      if (body.action === 'preparar_generacion_semana') return {ok:true,generation:{currentWeek:'2026-09-14',nextWeek:'2026-09-21'}};
      if (body.action === 'check_week_closure') return {ok:true,yaCerrada:false};
      assert.equal(body.action, 'obtener_confirmacion_disponibilidad');
      return availabilityApi.readAvailabilityConfirmation(db, 'fixture');
    };
    const context = { modoEntrada: 'coach', codigoUsuario: 'fixture', texto: 'Genera mi semana',
      setMostrarBotonNuevaSemana() {}, setCargando() {}, apiCall, weeklyTemporalIntentRef: { current: null },
      weeklyPlanningContinuationRef:{current:null},availabilityConfirmationRef: ref, setEsperandoConfirmacionDisponibilidad: v => { waiting = v; },
      setMensajes: fn => { messages.splice(0, messages.length, ...fn(messages)); },
      orquestarGeneracionSemana: () => { throw Error('CHECKPOINT_REQUIRED'); } };
    const execute = vm.runInNewContext(compile(`const dispararGeneracion=${entry.getText(ast)};async function run() ${body};run;`), context);
    await execute(); assert.equal(waiting, true); assert.ok(ref.current);
    assert.match(messages[0].content, /correcta para esta semana/);
    assert.equal(actions.filter(a => a === 'obtener_confirmacion_disponibilidad').length, 1);
  });
}
test('shared availability checkpoint fails closed on read failure', async () => {
  const entry = find(ast, n => ts.isVariableDeclaration(n) && n.name.getText(ast) === 'dispararGeneracion').initializer;
  let waiting = true;
  const execute = vm.runInNewContext(compile(`const run=${entry.getText(ast)};run;`), {
    codigoUsuario: 'fixture', weeklyPlanningContinuationRef:{current:null},weeklyTemporalIntentRef: { current: null }, availabilityConfirmationRef: { current: 'old' },
    apiCall: async () => ({ ok: false }), setEsperandoConfirmacionDisponibilidad: v => { waiting = v; }, setMensajes() {},
    orquestarGeneracionSemana: () => { throw Error('MUST_NOT_ADVANCE'); },
  });
  await execute(); assert.equal(waiting, false);
});
