...
test('all known direct sports writes are in planPersistence; public account bypasses are removed',()=>{
  const results=[];
  ...
  assert.deepEqual(sports.map(r=>[r.path,r.method]).sort(),[
    ['../../lib/planning/planPersistence.ts','insert'],['../../lib/planning/planPersistence.ts','update']].sort());
  assert.deepEqual(results.filter(r=>r.table==='tabla'),[]);
});
