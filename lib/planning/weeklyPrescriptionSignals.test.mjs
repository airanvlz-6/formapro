...
test('requiredPattern squat never admits another pattern even if it is executable',()=>{
  assert.deepEqual(plain(patterns(build(fixture({material:['Mancuernas']},'squat')))),['squat']);
  const r=weekly.buildAllowedWeeklyPlanContract(fixture({},'squat'));
  assert.equal(r.ok,false);assert.ok(r.errors.includes('NO_VALID_EXECUTABLE_REST_ARRANGEMENT'));
});
