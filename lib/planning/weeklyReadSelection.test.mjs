...
for(const [label,datos,instant,expected] of [
  ...
  ['Canary Sunday while Madrid Monday',undefined,'2026-09-06T22:30:00Z','2026-08-31']]) {
...
async function refreshFlow(saved={ok:true,persistenceStatus:'committed'},target='2026-09-07'){
  ...
  const globals={estructura:{calendarReceipt:'calendar'},codigoUsuario:'test',weeklyGeneration:{token:'captured'},planCompleto:{week_start:target},console:{log(){}},
...
test('Sunday closed -> next-week committed -> actual refresh -> Mi Plan mount queries/displays next Monday',async()=>{
  ...
  const assignment=find(web,n=>ts.isBinaryExpression(n)&&n.left.getText(web)==='continuation'&&ts.isObjectLiteralExpression(n.right))[0].right.getText(web);
  const continuation=evaluate(`(${assignment});`,{codigoUsuario:'test',closure:{yaCerrada:true},generation:{nextWeek:'2026-09-07',currentWeek:'2026-08-31'}});
  const target=evaluate(`${selection};`,{continuation});
...
for(const status of ['conflict','error','unknown'])test(`${status} performs no resave and selects no successful target`,async()=>{
  const r=await refreshFlow({ok:false,persistenceStatus:status,retryable:false});assert.equal(r.result.ok,false);assert.equal(r.result.canContinue,false);assert.equal(r.selected,undefined);assert.equal(r.calls.filter(c=>c.action==='guardar_plan_semana').length,1);
});
...
for (const [instant, week, day] of [
 ['2026-09-06T22:59:00+01:00','2026-08-31','domingo'],
 ['2026-09-06T23:30:00+01:00','2026-08-31','domingo'],
 ['2026-09-06T23:59:00+01:00','2026-08-31','domingo'],
 ['2026-09-07T00:00:00+01:00','2026-09-07','lunes'],
]) test(`Canary weekly boundary ${instant}`, async () => {
 assert.equal(resolveCompletionDate(instant).weekStart, week);
 assert.equal(resolveCompletionDate(instant).day, day);
 assert.equal((await reader(undefined,instant)).filters.week_start, week);
});
