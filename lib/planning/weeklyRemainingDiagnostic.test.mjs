...
import { sportsRuntime, fakeDatabase, plain, compile, equippedProfileFixture } from '../sports/trainingContractTestRuntime.mjs';
const events = [];
...
function fixture(blocked = false) {
  const tables = { usuarios: { modo_entrada:'coach', categoria:'funcional', especialidad:'funcional_crossfit', objetivo_principal:'CrossFit Games',
    perfil:equippedProfileFixture(), workout_history:[], ciclo_actual:{bloque:'deload'}, distribucion_semanal:availability },
...
test('real adapter, controlled restrictions: rejection exposes calendar, method stages and no selected plan', async()=>{
  ...
  for(const [id,method] of [['recuperacion_activa','running_recovery'],['tecnica','box_technique'],['fuerza_maxima','box_max_strength']]) {
    const a=d.adaptations.find(a=>a.adaptationId===id);assert.ok(a.candidateMethodsAfterTemporalCalendar.includes(method));
    assert.deepEqual(plain(a.candidateMethodsAfterFeasibility),[]);
  }
  assert.ok(d.rejectedIntents.length>5);assert.ok(d.rejectedIntents.every(r=>r.errorCodes.includes('MOVEMENT_POOL_EMPTY')));
});
test('same normalized inputs, empty restrictions restore goal alternatives; causal simulations never bypass feasibility',async()=>{
  ...
  const ids=[...new Set(Object.values(contract.dayOptions).flat().flatMap(o=>o.intent?.kind==='adaptation'?[o.intent.methodId]:[]))];
  for(const method of ['box_technique','running_recovery','box_max_strength'])assert.ok(ids.includes(method));
  const enumerate=(c)=>days.reduce((rows,day)=>rows.flatMap(row=>c.dayOptions[day].map(o=>[...row,o])),[[]]);
  const eligible=c=>enumerate(c).filter(row=>row.some(o=>!o.protected&&['TRAIN','RECOVERY'].includes(o.state))).length.toString();
  assert.equal(diagnostic.countRemainingSelections(contract).validSelectionCount,eligible(contract));
  ...
  assert.equal(diagnostic.countRemainingSelections(noRest).validSelectionCount,eligible(noRest));
  ...
  for(const method of ['box_technique','running_recovery']) {
    ...
    assert.equal(diagnostic.countRemainingSelections(forced).validSelectionCount,eligible(forced));
  }
...
test('D1 D4: viable result emits feasible methods and exact counts, without selecting a schedule',async()=>{
  ...
  const d=logs[0][1];assert.equal(d.reason,null);assert.ok(BigInt(d.validSelectionCount)>31n);
  const methods=d.adaptations.flatMap(a=>a.candidateMethodsAfterFeasibility);
  for(const id of ['box_technique','running_recovery','box_max_strength'])assert.ok(methods.includes(id));
...
test('L1 L2 L3: rejection codes, restriction entries and method pipeline are visible in flat JSON lines',async()=>{
  ...
  const pipeline=detailRows('WEEKLY_METHOD_PIPELINE_DETAIL');assert.equal(pipeline.length,input.strategy.adaptations.length);
  assert.ok(pipeline.some(p=>p.initialMethodIdsCsv==='running_recovery'));assert.ok(pipeline.some(p=>p.afterFeasibilityIdsCsv==='box_technique'));
