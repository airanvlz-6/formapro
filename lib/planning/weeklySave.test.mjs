...
const protectedCalendarSessionIndices = sportsRuntime()('../planning/weeklyCalendar').protectedCalendarSessionIndices;
...
function nextWeek() {
  const date = new Date(new Date().toLocaleDateString('en-CA', { timeZone: 'Atlantic/Canary' }) + 'T12:00:00');
...
async function backend(options = {}) {
  ...
  const supabase = { from(table) {
    ...
    const query = {
      ...
      then(resolve,reject) { return Promise.resolve().then(() => {
        ...
        if(table==='weekly_plan_generation_log') {
          if(options.generationReadThrow) throw new Error('generation log transport');
          if(options.generationReadError) return {count:null,error:{code:'42P01',message:'missing generation log'}};
          return {count:options.generationCount || 0,error:null};
        }
        if(table==='usuarios') return {data:{modo_entrada:'planificacion',ciclo_actual:{semana:4,totalSemanas:4,bloque:'Old'},workout_history:[]}};
        return {count:options.generationCount || 0};
      }).then(resolve,reject); }
    }; return query;
  }};
  let generation;
  try { generation=await beginWeeklyGeneration(supabase,'test',new Date().toLocaleDateString('en-CA',{timeZone:'Atlantic/Canary'})); }
  ...
  const execute = executable(actionBody, {
    // Sports authority is an orthogonal seam here; adversarial real-gate tests live in sports.
    admittedWeekObjective: (_receipt, _user, _week, legacy) => legacy,
    prepareCompletedBlockOutcome: sportsRuntime()('../planning/completedBlockOutcome').prepareCompletedBlockOutcome,
    wholeWeekFailure:sportsRuntime()('../planning/wholeWeekFailure').wholeWeekFailure,
    enforceWholeWeek: async (_u,_w,rows,sources)=> options.repairFailure
      ? {ok:false,code:'WEEK_REPAIR_FAILED',result:{status:'repair_required',diagnostics:[{code:'WEEK_EXACT_DUPLICATE',severity:'ERROR',sessionIds:['martes:1'],repairability:'same_contract'}]},repairCount:1,orchestration:{finalStatus:'repair_required',stages:[{stage:'local',targetIds:['martes:1'],failedIds:['martes:1'],failedReasons:[{id:'martes:1',reason:'REPAIR_FAILED'}]}]}}
      : options.weekInvalid ? {ok:false,code:'WEEK_COHERENCE_INVALID',result:{status:'invalid'}} : {ok:true,sessions:rows,sessionEvidence:sources,result:{status:'pass',diagnostics:[]},repairCount:0},
    validateAdmittedWholeWeek:()=>{if(options.finalWeekThrow)throw new Error('final validator');return {status:options.finalWeekInvalid?'invalid':'pass',diagnostics:[]};},
    issueWholeWeekReceipt:()=>{if(options.receiptThrow)throw new Error('receipt issue');return 'signed-week-receipt';},
    assertWeeklyCalendar: async () => ({evidence:options.longitudinal ? {coherenceVersion:1,longitudinal:options.longitudinal} : {},contexts:{}}), admitSessionContent: session => session, assertFreshSessionRestrictions: async () => { if(options.freshnessError) throw new Error("SESSION_RESTRICTIONS_CHANGED_REGENERATE"); }, assertCurrentPrescriptionScope: async () => {},
    structuredClone, supabase,codigo:'test',datos:{plan,weeklyContractVersion:options.weeklyContractVersion,generationToken: options.invalidToken ? 'invalid' : generation.token},
    weeklySaveAdmission:(_receipt,_user,_week,sessions)=>({outcome:options.noPrescription || null,
      survivorIndices:options.survivorIndices ?? (options.weeklyContractVersion === 1 ? protectedCalendarSessionIndices(sessions) : [])}),
...
for(const option of ['weekInvalid','finalWeekInvalid'])test(`${option}: actual save route blocks all writes`,async()=>{
  const r=await backend({[option]:true});assert.equal(r.response.body.ok,false);assert.equal(writes(r).length,0);
});
test('whole-week repair failure is reported before canonical write with explicit non-commit state',async()=>{
  const r=await backend({repairFailure:true});
  assert.equal(r.response.status,200);assert.equal(r.response.body.ok,false);
  assert.equal(r.response.body.code,'WEEK_REPAIR_FAILED');
  assert.equal(r.response.body.saveStage,'whole_week_repair');
  assert.equal(r.response.body.persistenceStatus,'not_committed');
  assert.equal(r.response.body.commitConfirmed,false);
  assert.deepEqual(r.response.body.repairDiagnostics,[{code:'WEEK_EXACT_DUPLICATE',severity:'ERROR',sessionIds:['martes:1'],repairability:'same_contract'}]);
...
test('create uses fresh server UUIDs, revision 1 and INSERT after strict receipt',async()=>{
  ...
  assert.equal(r.response.body.commitConfirmed,true);
  assert.deepEqual(r.response.body.persistenceReceipt,{planId:'new-plan',revision:1,weekStart:plan.week_start});
...
test('create commit precedes outcome, generation log and event; save cannot advance the canonical cycle',async()=>{
  const r=await backend();assert.deepEqual(writes(r).map(w=>w.table),['weekly_plan','block_outcomes','weekly_plan_generation_log','weekly_plan_events']);
  assert.ok(!writes(r).some(w=>w.table==='usuarios'));
});
test('actual save ignores competing client counters and persists the admitted longitudinal projection', async () => {
  const plan = proposal();
  const r = await backend({ plan, longitudinal: { weekStart: plan.week_start, bloque: 'deload', semana: 1, totalSemanas: 2 } });
  assert.equal(r.response.body.ok, true, JSON.stringify(r.response));
  assert.equal(writes(r)[0].payload.week_number, 1);
  assert.equal(writes(r)[0].payload.total_weeks_block, 2);
  assert.equal(writes(r)[0].payload.block_name, 'deload');
  assert.ok(!writes(r).some(w => w.table === 'usuarios'));
});
for (const option of ['generationReadError', 'generationReadThrow']) test(`generation quota admission ${option} is deterministic and pre-write`, async () => {
  const r = await backend({[option]:true});
  assert.equal(r.response.status, 200);
  assert.equal(r.response.body.ok, false);
  assert.equal(r.response.body.code, 'GENERATION_LOG_READ_FAILED');
  assert.equal(r.response.body.saveStage, 'generation_quota_admission');
  assert.equal(writes(r).length, 0);
});
for (const [option, code, stage] of [['finalWeekThrow','WEEK_FINAL_VALIDATION_FAILED','final_validation'],['receiptThrow','WEEK_RECEIPT_ISSUE_FAILED','receipt']]) test(`${stage} failure is deterministic and pre-write`, async () => {
  const r = await backend({[option]:true});
  assert.equal(r.response.body.ok, false); assert.equal(r.response.body.code, code); assert.equal(r.response.body.saveStage, stage);
  assert.equal(writes(r).length, 0);
});
async function runFrontend(response) {
  const calls=[];let reloads=0;const planCompleto={week_start:nextWeek()};
  const execute=executable(frontendBody,{estructura:{calendarReceipt:"calendar"},planCompleto,weeklyGeneration:{token:'snapshot-token'},codigoUsuario:'test',
...
for(const response of [{ok:false,error:'PLAN_REVISION_CONFLICT'},{ok:false,error:'PLAN_PERSISTENCE_UNKNOWN'},undefined,{ok:'true'}])
  test(`Orchestrator stops without replay for ${response?.error || 'unconfirmed'}`,async()=>{
    const r=await runFrontend(response);assert.equal(r.result.ok,false);assert.equal(r.result.canContinue,false);assert.equal(r.calls.length,1);assert.equal(r.reloads,1);
...
test('Orchestrator committed save is terminal: no verification-driven replay',async()=>{
  const r=await runFrontend({ok:true});assert.deepEqual(plain(r.result),plain(r.planCompleto));assert.equal(r.calls.length,1);
});
test('Orchestrator preserves the pre-commit repair stage and does not claim persistence',async()=>{
  const r=await runFrontend({ok:false,code:'WEEK_REPAIR_FAILED',saveStage:'whole_week_repair',persistenceStatus:'not_committed',commitConfirmed:false});
  assert.equal(r.result.code,'WEEK_REPAIR_FAILED');assert.equal(r.result.saveStage,'whole_week_repair');
  assert.equal(r.result.persistenceStatus,'not_committed');assert.equal(r.result.commitConfirmed,false);
  assert.equal(r.reloads,1);assert.equal(r.calls.length,1);
});
...
test('restriction freshness failure stops actual weekly save after validation but before DB mutation',async()=>{
 const r=await backend({freshnessError:true});
 assert.equal(r.response.body.ok,false);
 assert.equal(r.response.body.code,'SESSION_RESTRICTIONS_CHANGED_REGENERATE');
 assert.equal(writes(r).length,0);
});


test('weekly contract save uses server snapshot indices to preserve pending recovery and REST identity',async()=>{
  const existing=snapshot();
  existing.sessions[0].tipo='carrera';existing.sessions[0].stimulusId='recuperacion_activa';
  const plan=proposal();plan.week_objective='Generic authorized stimuli';
  plan.sessions=structuredClone(existing.sessions);
  const r=await backend({existing,plan,weeklyContractVersion:1,survivorIndices:[0,1]});
  assert.equal(r.response.body.ok,true,JSON.stringify(r.response));
  assert.deepEqual(writes(r)[0].payload.sessions,existing.sessions);
  assert.equal(r.queries.filter(q=>q.table==='weekly_plan'&&q.method==='select'&&q.captured).length,0);
});

test('R1 R9 R10: terminal no-prescription stops all save effects and preserves snapshot',async()=>{
 const existing=snapshot(),before=JSON.stringify(existing);
 const outcome={ok:false,code:'NO_NEW_EXECUTABLE_PRESCRIPTION',reason:'NO_FEASIBLE_REMAINING_SELECTION',canContinue:false};
 const r=await backend({existing,noPrescription:outcome});
 assert.deepEqual(r.response.body,outcome);assert.equal(writes(r).length,0);
 assert.equal(JSON.stringify(existing),before);
 assert.equal(r.queries.filter(q=>q.table==='weekly_plan_generation_log').length,0);
});
test('orchestrator transports no-prescription and generation cap without null conversion',async()=>{
 for(const code of ['NO_NEW_EXECUTABLE_PRESCRIPTION','MAX_GENERATIONS_REACHED','WEEKLY_REGENERATION_NO_OP']) {
  const r=await runFrontend({ok:false,code,canContinue:false});
  assert.equal(r.result.code,code);assert.equal(r.result.canContinue,false);assert.equal(r.calls.length,1);
 }
});

test('Repair 1 leaves the two-generation guard unchanged for an effective proposal',async()=>{
 for(const count of [0,1,2]){
  const plan=proposal();plan.week_objective='Changed';
  const r=await backend({existing:true,plan,generationCount:count});
  assert.equal(r.response.status,count===2?403:200);
  if(count===2){assert.equal(r.response.body.reason,'MAX_GENERATIONS_REACHED');assert.equal(writes(r).length,0);}
  else assert.ok(writes(r).some(w=>w.table==='weekly_plan_generation_log'&&w.payload.version===count+1));
 }
});
