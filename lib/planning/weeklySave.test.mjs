...
async function backend(options = {}) {
  ...
  const execute = executable(actionBody, {
    ...
    enforceWholeWeek: async (_u,_w,rows,sources)=> options.weekInvalid ? {ok:false,code:'WEEK_COHERENCE_INVALID',result:{status:'invalid'}} : {ok:true,sessions:rows,sessionEvidence:sources,result:{status:'pass',diagnostics:[]},repairCount:0},
