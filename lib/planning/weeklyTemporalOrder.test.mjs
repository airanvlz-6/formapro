...
import {sportsRuntime,fakeDatabase,compile,plain,equippedProfileFixture} from '../sports/trainingContractTestRuntime.mjs';
...
for(const includeToday of [true,false])test(`T2 T3 T9: explicit ${includeToday} resolves facts before open proposal, not catalog feasibility`,async()=>{
 for(const blocked of [false,true]){
  ...
  assert.equal(r.preflightRequirement,undefined);assert.equal(r.canContinue,true);
  if(!blocked) {
   ...
   assert.deepEqual(plain(domain),{lunes:['REST'],martes:['TRAIN'],miercoles:['REST','RECOVERY','TRAIN'],jueves:['REST','TRAIN'],viernes:['REST','TRAIN'],sabado:['REST','TRAIN'],domingo:['REST','RECOVERY','TRAIN']});
  }
 }
});
 ...
 const ctx={codigoUsuario:'fixture',Date:FixedDate,structuredClone,console:{log(){},error(){}},planBlockLabel:load('planPresentation').planBlockLabel,
  ...
  fetch:async(_url,init)=>{const b=JSON.parse(init.body);actions.push(b.action);requests.push(b);let r;
   switch(b.action){
    ...
    case 'planificar_semana':r={ok:true,estructura:{weeklyContractVersion:2,calendarReceipt:'calendar',strategy:{adaptacion_principal:'Technique'},sessions:days.map((dia,i)=>i===0||i===1?{...f.snapshot.sessions[i],weeklyProtected:true}:i===3?{dia,tipo:'box',stimulusId:'tecnica'}:{dia,tipo:'descanso'})}};break;
...
test('direct Planner request cannot default missing temporal authority to true',async()=>{
 ...
 for(const value of [undefined,null,'true']){
  ...
  const execute=vm.runInNewContext(compile(`async function run() ${branch.getText(source)};run;`),{
   datos:{weeklyContractVersion:2,targetWeekStart:week,empezarHoy:value},codigo:'fixture',supabase:f.db,
