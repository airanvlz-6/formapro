...
function fixture(){
  ...
  const contracts=['martes','jueves'].map(targetDay=>{
    const r=load('allowedTrainingContract').buildAllowedTrainingContract(contractFixture({targetWeekStart:week,targetDay,intent,doseContext}));
  ...
  const proposals=contracts.map(c=>({schemaVersion:2,stimulusId:c.stimulusId,structureId:'strength_sets',blocks:[
    {blockType:'warmup',movements:[{movementId:'back_squat',prescription:{durationSeconds:120,intensity:{kind:'rpe',value:3}}}]},
    {blockType:'main',movements:[{movementId:'back_squat',prescription:{sets:4,reps:5,restSeconds:120,intensity:{kind:'rpe',value:7}}}]}]}));
...
test('unchanged duplicate from repair fails closed after one call',async()=>{
  ...
  assert.equal(r.ok,false);assert.equal(r.code,'WEEK_REPAIR_FAILED');assert.equal(calls,1);
});
for(const [name,mutate] of [
  ...
  assert.equal(r.ok,false);assert.equal(calls,1);assert.equal(r.code,'WEEK_REPAIR_FAILED');
});
...
test('forged source receipt cannot be used for bounded repair',async()=>{
  const f=fixture();f.sources[1].sessionReceipt+='x';let calls=0;
