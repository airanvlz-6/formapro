...
async function endpoint(action, db, preparation = (db, user) => prepare(db, user, today)) {
  const names = ['supabase', 'codigo', 'prepareCanonicalReadiness', 'calcularReadiness', 'scoreAForgeState', 'combinarConCheckinSubjetivo', 'calcularFrecuenciaRealRelativa', 'evaluarRelevanciaContextual', 'NextResponse', 'contextualSessionIntensity', 'legacyDurationMinutes'];
  ...
  return new Function(code + '; return invoke;')()(db, 'TEST', preparation, calculate, scoreAForgeState, combinarConCheckinSubjetivo,
    ...
    load('lib/sports/sessionPresentation.ts').contextualSessionIntensity, load('lib/sports/sessionPresentation.ts').legacyDurationMinutes);
}
