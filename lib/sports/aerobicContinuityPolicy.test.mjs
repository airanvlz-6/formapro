...
test('coach preflight uses habitual as context without demanding exact-repeat confirmation',async()=>{
  ...
  assert.ok(methods.includes('running_base'));
...
test('production weekly receipt -> Builder -> signed session -> freshness retains current interaction',async()=>{
  ...
  const planned=await load('../planning/prepareAllowedWeeklyPlanContract').planBoundedWeek(db,'fixture',{
    ...
    const selections=Object.entries(c.dayOptions).map(([day,options])=>{
      ...
      if(option.intent)chosen.add(option.intent.adaptationId);return {day,optionId:option.optionId,decision:{role:option.state==='REST'?'RECOVERY':'PRIMARY',reason:'Fixture: continuidad aeróbica autorizada.'}};
    });
  ...
  const result=await server.generateTrainingSession(db,'fixture',{targetWeekStart:active.targetWeekStart,day:slot.dia,discipline:'carrera',stimulus:'base_aerobica',
    ...
    return JSON.stringify({schemaVersion:2,stimulusId:c.stimulusId,structureId:'continuo_carrera',explanation:'Mantener la exposición habitual de 45 minutos.',blocks:[{blockType:'main',movements:[{
      movementId:'rodaje_z2',prescription:{durationSeconds:2700,
  ...
  assert.ok(builderPrompt.split('Contexto no autoritativo:')[1].includes('habitualDeclarations'));
