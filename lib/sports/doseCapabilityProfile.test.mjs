...
test('G H I J K: 30 km, 45 minutes and 4 sessions coexist without conversions, products or targets', () => {
  ...
  for (const p of load('runningMethodDosePolicies').RUNNING_METHOD_DOSE_POLICIES) {
    ...
    assert.equal(p.selectDose, null); assert.equal(r.status, 'UNRESOLVED'); assert.equal(r.dose, null);
  }
});
...
test('R S: readiness of declarations never claims a quantifiable dose', () => {
  assert.equal(aerobic(cap()).doseCapability, 'EVIDENCE_READY_POLICY_MISSING');
  assert.equal(aerobic(cap()).policyStatus, 'NOT_ESTABLISHED');
...
test('X Y: capture and evidence cannot grant Supervision or external Focus prescription authority', () => {
  for (const s of [{mode:'supervision',prescriptionAllowed:false,managedDisciplines:[],externalDisciplines:['carrera']},
    ...
    assert.equal(aerobic(c).doseCapability,'EVIDENCE_READY_POLICY_MISSING'); assert.ok(c.entries.every(e=>!e.prescriptionAllowed));
...
test('AC: stable deduplication and recency UNKNOWN even for an old declaration', () => {
  ...
  assert.equal(aerobic(cap(b)).doseCapability,'EVIDENCE_READY_POLICY_MISSING');
});
...
test('AD: dedicated JSON writer; generic profile update preserves the declarations', () => {
  ...
  assert.match(route,/\['prescription_signals', 'prescription_access', 'runningHabitualDeclarations'\]/);
...
test('real server read pipeline produces capability insufficiency and contextual requirement without any writes', async () => {
  ...
  const first=await run(); assert.equal(first.code,'RUNNING_DOSE_CAPABILITY_INSUFFICIENT'); assert.equal(first.canContinue,false);
  assert.equal(first.runningHabitualRequirement.field,durationField);
  tables.usuarios.perfil=profile(declarations(),tables.usuarios.perfil);
  const second=await run(); assert.equal(second.code,'RUNNING_DOSE_CAPABILITY_INSUFFICIENT'); assert.equal(second.runningHabitualRequirement,null);
  assert.equal(aerobic(second.doseCapabilities).doseCapability,'EVIDENCE_READY_POLICY_MISSING');
  let plannerCalls=0;
  const planned=await load('../planning/prepareAllowedWeeklyPlanContract').planBoundedWeek(db,'fixture',
    {...request,empezarHoy:false,strategyVersion:1},async()=>{plannerCalls++;throw Error('NO_PLANNER');});
  assert.equal(planned.code,'RUNNING_DOSE_CAPABILITY_INSUFFICIENT'); assert.equal(plannerCalls,0);
});
...
test('contextual UI answers only through the structured capture action, then clears the pending question', async () => {
  ...
  const run=vm.runInNewContext('(async()=>{'+code+'})',{
    ...
    setCargando(){},setInput(){},setMensajes(fn){messages.push(...fn([]));},setPendingRunningHabitualQuestion(v){pending.push(v);},
