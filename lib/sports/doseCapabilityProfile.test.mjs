...
test('missing running dose stays excluded while feasible strength can continue in deload without writes', async () => {
  ...
  assert.equal(aerobic(context.input.doseCapabilities).doseCapability,'RECONFIRMATION_REQUIRED');
  ...
  assert.ok(prepared.contract.strategy.deferred.some(d=>d.reference.includes('running_base')));
});
