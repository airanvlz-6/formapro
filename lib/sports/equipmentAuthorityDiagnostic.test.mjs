...
test('safe serialization excludes raw profile, arbitrary source, unknown IDs and invalid identity', () => {
  ...
  assert.deepEqual(Object.keys(lines[1]).sort(), ['event','planningRunId','day','rawEnvironmentPresent','rawRoomTypePresent','rawMaterialPresent','resolvedEnvironment','sessionEnvironmentSource','assignedDiscipline','resolutionReason','totalCount','truncated','equipmentId','environmentCompatibility','availableByEnvironment','finalEquipmentState','provenance'].sort());
});
...
test('real session boundary: enabled/failed logging adds zero DB reads or feasibility evaluations', async () => {
  for (const perfil of [{}, { lugar_entreno: 'BOX' }]) {
    ...
    assert.equal(results[0].builderCalls, perfil.lugar_entreno ? 2 : 0);
  }
});
