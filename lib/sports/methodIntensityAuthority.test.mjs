...
test('A/B/H capability or maxHR alone creates no zone; C2 only supplies subjective fallback', () => {
  for (const profile of [device, { ...device, fc_max: 190, fc_reposo: 50 }, { ...device, fc_max_metodo: 'formula_edad', edad: 40 }]) {
    ...
    const a = api.resolveMethodIntensity(c); assert.equal(a.status, 'RESOLVED'); assert.ok(a.targets.every(t => t.primary.kind === 'rpe'));
...
test('H unresolved retains compatible legacy behavior and emits explicit non-authoritative diagnostic', async () => {
  logs.length = 0; const { c, p } = fixture(); c.intensityAuthority = api.resolveMethodIntensity(c, { ...policy(hr), targets: [] });
...
test('binding rejects changed signals and incomplete derived provenance; logger failure cannot change success', async () => {
  ...
  c.intensityAuthority = api.resolveMethodIntensity(c, policy({ kind: 'rpe', value: 7, max: 8 }));
