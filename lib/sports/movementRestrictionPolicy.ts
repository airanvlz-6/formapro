...
export function movementRestrictionProperty(movement: Movimiento, property: BiomechanicalProperty,
  resolvedProperties?: Partial<Record<BiomechanicalProperty, boolean>>): boolean | 'unknown' {
  const evidence = (resolvedProperties ?? MOVEMENT_RESTRICTION_EVIDENCE[movement.id]?.properties)?.[property];
...
export function evaluateMovementRestrictions(movement: Movimiento, flags: readonly RestrictionFlag[],
  resolvedProperties?: Partial<Record<BiomechanicalProperty, boolean>>) {
  const incompatible: RestrictionFlag[] = [], unknown: RestrictionFlag[] = [];
  for (const flag of flags) {
    const property = movementRestrictionProperty(movement, RESTRICTION_FLAGS[flag], resolvedProperties);
