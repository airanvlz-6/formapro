...
test('explicit full box establishes conventional equipment including rower',()=>{
  const p=project({categoria:'box',perfil:{lugar_entreno:'Box CrossFit (equipamiento completo)'},workout_history:[{movementId:'remo'}]});
  assert.equal(p.prescriptionSignals.signals['equipment.remo'].state,'available');
  assert.equal(p.prescriptionSignals.signals['equipment.mancuerna'].state,'available');
});
