...
export function testDoseProposal(load, c) {
  const d = c.runningMethodDose.dose ?? testSelection(c.intent.methodId), s = d.structureConstraints;
  ...
  return { schemaVersion: 2, stimulusId: c.stimulusId, structureId: d.structures[0],
    ...(c.doseContext?.sessionDecisionAuthority === 'coach' ? { explanation: 'Dosis elegida por el proveedor de prueba dentro del contrato.' } : {}),
...
/** Isolated accepted numeric selectors for calendar/feasibility regressions downstream of capability.
 * Leaves actual feasibility, restrictions and calendar evaluation untouched. NOT production policy. */
export function numericPolicyFixture(path, exports) {
  if (path.endsWith('runningMethodDosePolicies.ts')) return { ...exports,
    RUNNING_METHOD_DOSE_POLICIES: exports.RUNNING_METHOD_DOSE_POLICIES.map(p => ({ ...p, policyId: 'test_' + p.policyId, selectDose: () => testSelection(p.methodId) })) };
  return exports;
}
