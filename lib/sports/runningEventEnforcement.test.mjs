...
test('real history remains factual context while coach capability needs no fixed dose',async()=>{
 ...
 const base=context.input.doseCapabilities.entries.find(e=>e.methodId==='running_base');assert.equal(base.prescriptionAllowed,true);assert.equal(base.longitudinalDose.selectedDose,null);
...
test('positive canonical declaration produces bounded week, Builder call and validated exact dose',async()=>{
 ...
 const selections=Object.entries(f.prepared.contract.dayOptions).map(([day,opts])=>({day,optionId:opts.find(o=>day==='lunes'&&o.intent?.methodId==='running_base')?.optionId??opts.find(o=>o.state==='REST').optionId,decision:{role:day==='lunes'?'PRIMARY':'RECOVERY',reason:'Fixture: preparación de evento dentro del contrato.'}}));
...
test('long-run REQUIRED can use coach dose selection within canonical constraints',async()=>{
...
test('quality allowed still needs C2 and B3; forbidden quality cannot acquire C2 authority',async()=>{
 ...
 assert.equal(threshold.weeklyIntensityEligibility,'ALLOWED');assert.equal(threshold.intensityStatus,'RESOLVED');assert.equal(threshold.prescriptionAllowed,true);
...
test('taper orchestration preserves recovery requirements while coach selects its dose',async()=>{
 ...
 const built=await p.prepareAllowedWeeklyPlanContract(f.db,'u',f.request);assert.equal(built.ok,true,JSON.stringify(built));
