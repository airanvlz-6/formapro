...
test('A-D v2 registry has six distinct families and no numeric training constants, shares or universal minima', () => {
  const registry = production('runningMethodDosePolicies').RUNNING_METHOD_DOSE_POLICIES;
  assert.equal(new Set(registry.map(p => p.family)).size, 6);
  for (const p of registry) { assert.equal(p.version, 2); if(p.methodId==='running_base'){assert.equal(typeof p.selectDose,'function');assert.equal(p.policyId,'running_base_declared_habitual_duration_v1');}else{assert.equal(p.selectDose,null);assert.match(p.policyId,/_v2$/);} }
...
for (const method of methods) test(`E/G-K ${method}: weekly declaration and generic quantities never select dose`, () => {
  for (const facts of [[declared()], [actual()], [actual(30000, 'distanceMeters')], [declared(), actual()]]) {
    ...
    assert.equal(a.reason, method === 'running_economy' ? 'POLICY_NOT_ESTABLISHED' : 'MISSING_COMPATIBLE_EVIDENCE');
  }
});
...
test('Z real server emits unresolved v2 for first-plan HM with declaration, no pace, RPE fallback', async () => {
  let issued, calls = 0;
...
test('v2 server signing, receipt tamper/freshness and evidence change (isolated accepted test policy)', async () => {
  ...
  const tampered = structuredClone(result.sesion); tampered.structuredPrescription.runningMethodDose.dose.maximumAuthorized++;
