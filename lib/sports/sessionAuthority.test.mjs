...
import { equippedProfileFixture, sportsRuntime, contractFixture, fakeDatabase, compile, plain, completeDoseFixture } from './trainingContractTestRuntime.mjs';
...
const tables = { usuarios: { modo_entrada: 'planificacion', categoria: 'box', perfil: equippedProfileFixture() }, weekly_plan: [], athlete_training_sources: [], external_training_records: [] };
...
test('changed benchmark or time budget revokes fresh session dose before save', async () => {
  for (const change of [p => p.perfil.duracion = '45 min', p => p.test_atleta = { back_squat: 150 }]) {
    const profile = { modo_entrada: 'planificacion', categoria: 'box', perfil: { ...equippedProfileFixture(), duracion: '60 min' } };
...
test('temporary date unavailability revokes an already issued standalone session receipt', async () => {
  const profile = { modo_entrada: 'planificacion', categoria: 'box', perfil: equippedProfileFixture() };
  const database = db({ usuarios: profile }), out = await issue(database);
  assert.equal(out.ok, true, JSON.stringify(out));
  profile.perfil.prescription_access = { '2026-09-01': { availability: 'unavailable' } };
  await assert.rejects(() => assertFreshSessionRestrictions(database, 'u', request.targetWeekStart, out.sesion), /SESSION_TEMPORARY_AVAILABILITY_CHANGED/);
});
...
test('scope revocation blocks a previously valid receipt without replacing original contract', async () => {
  ...
  await assert.rejects(assertCurrentPrescriptionScope(db({ usuarios: { modo_entrada: 'supervision', categoria: 'box', perfil: equippedProfileFixture() } }), 'u', 'box'), /SCOPE_REVOKED/);
});
...
for (const mode of ['planificacion', 'focus', 'supervision']) for (const invalid of [false, true])
  test(`actual forced generator ${mode}, invalid=${invalid}`, async () => {
    let calls = 0;
    const database = db({ usuarios: { modo_entrada: mode, categoria: 'box', perfil: equippedProfileFixture() }, athlete_training_sources: mode === 'focus' ? [{ disciplina: 'box', owner: 'forge', activo: true, dias: ['martes'] }] : [] });
...
function routeDB({ session, pending, mode = 'planificacion' } = {}) {
  ...
  const database = { writes, from(table) {
    ...
    const read = single => {
      ...
      if (table === 'usuarios') return { data: { modo_entrada: mode, categoria: 'box', perfil: equippedProfileFixture() }, error: null };
...
test('actual final weekly gate checks content after transformations and before identity admission', async () => {
  ...
  for (const invalid of [false, true]) {
    ...
    const r = await execute('guardar_plan_semana', { plan: { week_start: request.targetWeekStart, sessions } }, database, {
      ...
      weeklySaveAdmission:()=>({outcome:null,survivorIndices:[]}),
      ...
      assertWeeklyCalendar: async () => ({ evidence: {}, contexts: {} }), // Calendar integration is exercised in weeklyCalendar.test.mjs.
      enforceWholeWeek: async (_u,_w,rows,sources)=>({ok:true,sessions:rows,sessionEvidence:sources,result:{status:'pass',diagnostics:[]},repairCount:0}),
...
test('actual final weekly route rejects legacy calendar evidence before identity admission', async () => {
  ...
  const calendarDB = fakeDatabase({usuarios:{modo_entrada:'planificacion',categoria:'box',perfil:{...equippedProfileFixture(),dias:6},workout_history:[],distribucion_semanal:{box:days}},athlete_training_sources:[]});
  ...
  const r=await execute('guardar_plan_semana',{plan:{week_start:request.targetWeekStart,sessions},calendarReceipt},database,{
    weeklySaveAdmission:authority.weeklySaveAdmission,
    prepareWeeklyEntries:s=>s.map(session=>({kind:'new',session})),entrySession:e=>e.session,
    getCanonicalRestrictions:async()=>contractFixture().restrictionsSnapshot,
    buildFocusContext:async()=>({esModoFocus:false,disciplinasExternas:[]}),
