...
import { sportsRuntime, contractFixture, fakeDatabase, compile, plain } from './trainingContractTestRuntime.mjs';
...
const response = (prompt, invalid = false) => {
  ...
  return JSON.stringify({ stimulusId: c.stimulusId, structureId: c.allowedStructureIds[0], blocks: ['warmup', 'main', 'cooldown']
...
test('admission strips scientific notes, arbitrary metadata and transient evidence', async () => {
  ...
  assert.deepEqual(Object.keys(s).sort(), ['dia', 'tipo', 'titulo', 'por_que', 'descripcion', 'debilidad_relacionada'].sort());
...
function routeDB({ session, pending, mode = 'planificacion' } = {}) {
  ...
  const database = { writes, from(table) {
    ...
    const q = { select() { return q; }, eq() { return q; }, in() { return q; }, order() { return q; }, limit() { return q; }, range() { return q; }, gte() { return q; },
...
test('actual final weekly route rejects valid sports content that replaces signed REST', async () => {
  const generated = await issue();
  ...
  assert.equal(r.body.ok,false);assert.match(r.body.code,/CALENDAR_PROTECTED_DAY_CHANGED/);
