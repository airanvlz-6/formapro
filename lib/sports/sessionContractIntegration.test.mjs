...
test('actual Builder cannot return success for output outside the contract', async () => {
  ...
  assert.equal(result.body.ok, false); assert.equal(result.body.code, 'SESSION_CONTRACT_INVALID'); assert.equal(calls, 2);
});
...
test('actual UI callers settle loading and show a failure message once when the proposal rejects', async () => {
  ...
  const focusCaller = find(ast, n => ts.isArrowFunction(n) && n.body.getText(ast).includes('const planFocusInicial=await orquestarGeneracionSemana()')
  ...
  for (const caller of [focusAsync, chatCaller]) {
    ...
    const execute = vm.runInNewContext(compile(`const run = ${caller.getText(ast)}; run;`), {
      weeklyGenerationOutcomeMessage:load('../planning/weeklyRegeneration').weeklyGenerationOutcomeMessage,
      orquestarGeneracionSemana: async () => { calls++; return null; }, weeklyTemporalIntentRef: { current: undefined },
    ...
    await execute(undefined, false, true); assert.equal(pending, false); assert.equal(calls, 1);
...
test('actual UI callers present goal requirement without falsely announcing completion or failure', async () => {
  const ast = ts.createSourceFile('FormaPro.tsx', readFileSync('app/FormaPro.tsx', 'utf8'), ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
  const focus = find(ast, n => ts.isArrowFunction(n) && n.modifiers?.some(m => m.kind === ts.SyntaxKind.AsyncKeyword)
    && n.body.getText(ast).includes('const planFocusInicial=await orquestarGeneracionSemana()') && !n.body.getText(ast).includes('useEffect'));
  const chat = find(ast, n => ts.isVariableDeclaration(n) && n.name.getText(ast) === 'dispararGeneracion').initializer;
  for (const caller of [focus, chat]) {
    let pending = false, messages = [];
    const execute = vm.runInNewContext(compile(`const run = ${caller.getText(ast)}; run;`), {
      orquestarGeneracionSemana: async () => ({ goalRequirement: { state: 'goal_required' } }), weeklyTemporalIntentRef: { current: undefined },
      setGenerandoSemana: value => { pending = value; }, setMensajes: value => { messages = typeof value === 'function' ? value(messages) : value; },
      empezarHoyReal: true,
    });
    await execute(undefined, false, true); assert.equal(pending, false);
    assert.ok(!messages.some(m => /Semana generada y guardada|No se ha confirmado una semana nueva/.test(m.content)));
  }
});

test('actual API client retains structured goal continuation and displays server question once', async () => {
  const ast = ts.createSourceFile('FormaPro.tsx', readFileSync('app/FormaPro.tsx', 'utf8'), ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
  const declaration = find(ast, n => ts.isVariableDeclaration(n) && n.name.getText(ast) === 'apiCall');
  let pending, messages = [], calls = 0;
  const execute = vm.runInNewContext(compile(`const apiCall = ${declaration.initializer.getText(ast)}; apiCall;`), {
    codigoUsuario: 'u', pendingGoalQuestion: null,
    fetch: async () => { calls++; return { ok: true, json: async () => ({ ok: false, goalRequirement: { questionToken: 'signed', question: { text: '¿Qué objetivo priorizas?' } } }) }; },
    setPendingGoalQuestion: value => { pending = value; }, setMensajes: fn => { messages = fn(messages); },
    setTimeout: () => { throw Error('must not retry'); },
  });
  await execute({ action: 'planificar_semana', datos: { empezarHoy: false } });
  assert.equal(calls, 1); assert.equal(pending.empezarHoy, false); assert.equal(pending.token, 'signed'); assert.equal(messages.length, 1);
});
