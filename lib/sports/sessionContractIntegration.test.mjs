...
test('actual UI callers present goal requirement without falsely announcing completion or failure', async () => {
  ...
  const focus = find(ast, n => ts.isArrowFunction(n) && n.modifiers?.some(m => m.kind === ts.SyntaxKind.AsyncKeyword)
    && n.body.getText(ast).includes('const planFocusInicial=await orquestarGeneracionSemana(true)') && !n.body.getText(ast).includes('useEffect'));
  ...
  for (const caller of [focus, chat]) {
    ...
    const execute = vm.runInNewContext(compile(`const run = ${caller.getText(ast)}; run;`), {
      orquestarGeneracionSemana: async () => ({ goalRequirement: { state: 'goal_required' } }),
      setGenerandoSemana: value => { pending = value; }, setMensajes: value => { messages = typeof value === 'function' ? value(messages) : value; },
      empezarHoyReal: true,
    });
