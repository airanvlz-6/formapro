...
async function run(overrides = {}, options = {}) {
  let calls = 0;
  const profile = { modo_entrada: 'planificacion', categoria: 'box', athlete_development: [], perfil:{dias:6},workout_history:[],
...
test('actual valid Builder renders only structured IDs and returns server evidence', async () => {
  ...
  assert.ok(result.body.sesion.sessionReceipt); assert.match(result.body.sesion.descripcion, /repeticiones/); assert.ok(result.body.trainingContract.allowedMovementIds.length > 5);
