...
export function renderHumanSession(c: AllowedTrainingContract, p: StructuredSessionProposal) {
  const legacy = renderProfessionalSession(c, p), human = humanCoachingProjection(c, p);
  // At most six constant entity categories; arbitrary IDs/text are never logged.
  for (const entityType of human.fallbacks) {
    try { console.info?.('HUMAN_PRESENTATION_FALLBACK', { presentationVersion: 'human_v2', entityType, fallbackType: 'neutral_label' }); }
    catch { /* Diagnostics never change admission. */ }
  }
  return { ...legacy, titulo: human.title, por_que: human.why,
    structuredPrescription: { ...legacy.structuredPrescription, presentation: { version: 'human_v2' as const,
