...
export function legacySessionView<T extends TextSession>(session: T): T {
  ...
  const text = presentation?.version === 'human_v2' ? presentation.comparisonRepresentation : undefined;
...
export type PresentationVersion = 'legacy' | 'human_v2';
...
export function authenticatedPresentationVersion(value: unknown): PresentationVersion {
  ...
  if (value === 'human_v2') return value;
