...
test('unknown ID reaches movement resolution, not canonical shape rejection',()=>{
  ...
  assert.equal(api.inspectSessionRepresentation(p,true).ok,true);const r=validate(c,p);assert.equal(r.ok,true);assert.equal(load('movementVariants').resolvedMovement(p.blocks[1].movements[0]),undefined);
});
test('schema normalization does not admit false references, unknown fields or malformed doses',()=>{
  for(const modify of [m=>m.prescription.intensity={kind:'reference',value:75,referenceId:'invented'},m=>m.prescription.reps=-1,m=>m.prescription=[]]){
...
test('unknown family, operation and display remain Coach instructions without metadata fabrication',()=>{
  for(const mutate of [m=>m.variant.canonicalFamily='unknown',m=>m.variant.modifiers.direction='better',m=>m.variant.displayName='snatch']){
    const m=entry();mutate(m);assert.equal(validate(contract(),proposal(contract(),[m])).ok,true);assert.equal(load('movementVariants').resolvedMovement(m),undefined);
  }
});
test('resolved geometry and unknown compatibility remain executable under active restriction context',()=>{
  ...
  const r=validate(restricted,proposal(restricted));assert.equal(r.ok,true,JSON.stringify(r));
});
test('unknown grammar and modifiers emit safe diagnostics without spending a retry',async()=>{
  ...
  assert.equal(count,1);assert.equal(result.ok,true);
  const details=events.filter(x=>x[0]==='SESSION_SHAPE_VIOLATION').map(x=>x[1]);assert.deepEqual(details.map(x=>x.attempt),[1]);
...
test('actionable retry accepts a corrected Coach output',async()=>{
  ...
  const result=await load('sessionGeneration').generateContractSession(c,[],async prompt=>{
    if(count++) {assert.match(prompt,/JSON_INVALID/);return JSON.stringify(proposal(c));}
    return '{broken JSON';
