...
test('confirmed canonical field shapes pass unchanged under explicit modern schema',()=>{
  ...
  assert.deepEqual(plain(api.checkSessionShape(p,true).proposal),p);
});
test('isolated missing discriminator explains three legacy field failures, without claiming production discriminator',()=>{
  ...
  const details=[];const legacy=api.checkSessionShape(p,false,d=>details.push(d));
  ...
  assert.deepEqual(plain(api.checkSessionShape(r.proposal,true).proposal),plain(r.proposal));
});
test('additional entry fields can produce the same logged symptoms: production predicate still unknown',()=>{
  ...
  const details=[];const r=api.checkSessionShape(p,true,d=>details.push(d));assert.equal(r.ok,false);
...
for(const [code,mutate] of [
  ...
  assert.equal(api.checkSessionShape(p,true,d=>details.push(d)).ok,false);
...
test('identity aliases and dose aliases are diagnosed, never guessed',()=>{
  ...
  const details=[];assert.equal(api.checkSessionShape(p,true,d=>details.push(d)).ok,false);
...
test('unknown ID reaches movement resolution, not canonical shape rejection',()=>{
  ...
  assert.equal(api.checkSessionShape(p,true).ok,true);const r=validate(c,p);assert.equal(r.ok,false);assert.ok(r.violations.some(v=>v.startsWith('MOVEMENT_SEMANTICS_UNRESOLVED:')));
});
test('schema normalization does not admit false references, unknown fields or malformed doses',()=>{
  for(const modify of [m=>m.prescription.intensity={kind:'percent_1rm',value:75,referenceId:'invented'},m=>m.prescription.reps=-1,m=>m.prescription.privateField=42]){
  ...
  const p=canonicalIncident(contract());p.schemaVersion=1;assert.equal(api.checkSessionShape(p,true).ok,false);
});
...
test('canonical retry identifies the entry field rule and repairs without changing any exercise',async()=>{
  ...
  });assert.equal(result.ok,true,JSON.stringify(result));assert.equal(calls,2);assert.deepEqual(plain(result.proposal.blocks),good.blocks);
});
...
for(const [text,classification] of [['haz algo técnico','NON_EXECUTABLE_INSTRUCTION'],['3 x 8 y 10 burpees','HIDDEN_WORK'],['3 x 8 luego correr 90 min','HIDDEN_WORK'],['3 x 8 @ 140 bpm','OBJECTIVE_REFERENCE_UNRESOLVED'],['\n3 x 8','MALFORMED_OUTPUT']])test(`isolated ${classification}: remains hard`,()=>{
  const result=api.checkSessionShape(proposal(contract(),[entry(text)]),true);assert.equal(result.ok,false);
...
test('isolated shape rule reports typed paths without private keys or values',()=>{
  ...
  const seen=[];const r=api.checkSessionShape(proposal(contract(),[m]),true,d=>seen.push(d));
...
for(const [field,mutate] of [
  ...
  const r=api.checkSessionShape(proposal(contract(),[m]),true,d=>details.push(d));
...
test('separate movement diagnostics distinguish simultaneous codes; this is synthetic, not incident reconstruction',()=>{
  ...
  const details=[];const r=api.checkSessionShape(proposal(contract(),[a,b]),true,d=>details.push(d));
...
test('both violations on one movement survive the early dose return and observer failure',()=>{
  ...
  const p=proposal(contract(),[m]),r=api.checkSessionShape(p,true,d=>details.push(d));
  assert.equal(details.length,1);assert.equal(details[0].violationCodes.length,2);
  assert.deepEqual(plain(api.checkSessionShape(p,true,()=>{throw Error('observer');})),plain(r));
});
