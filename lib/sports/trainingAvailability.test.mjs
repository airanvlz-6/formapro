...
test('actual availability save rejects prose without writes and saves structured arrays',async()=>{
 ...
 for(const [descripcion,accepted] of [['solo entreno domingo',false],[JSON.stringify(raw),true]]) {
  const writes=[];const result=await vm.runInNewContext(compile('(async()=>'+branch.thenStatement.getText(source)+')()'),{
   datos:{descripcion},codigo:'test',normalizeAvailabilityForStorage:storage,NextResponse:{json:x=>x},console:{log(){}},
   supabase:{from:table=>({update:value=>({eq:async()=>{writes.push({table,value});return {};}})})}});
