...
export function sportsRuntime() {
  const cache = new Map();
  function load(file) {
    const path = resolve(file);
    if (cache.has(path)) return cache.get(path);
    const module = { exports: {} }; cache.set(path, module.exports);
    vm.runInNewContext(compile(readFileSync(path, 'utf8')), { module, exports: module.exports, structuredClone,
      require: name => { if (!name.startsWith('.')) throw new Error(`Unexpected dependency: ${name}`); return load(resolve(dirname(path), name + '.ts')); } });
