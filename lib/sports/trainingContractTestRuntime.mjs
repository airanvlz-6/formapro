...
export function sportsRuntime(globals = {}, transformModule) {
  const cache = new Map();
  function load(file) {
    ...
    if (transformModule) { module.exports = transformModule(path, module.exports); cache.set(path, module.exports); }
...
export function completeDoseFixture(c, p) {
  ...
  return { ...p, schemaVersion: 2, ...(c.doseContext?.sessionDecisionAuthority === 'coach' ? { explanation: 'Mantener la exposición con los datos disponibles.' } : {}), blocks: p.blocks.filter(b => b.blockType !== 'cooldown').map(b => ({ ...b,
