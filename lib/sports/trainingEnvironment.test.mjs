...
for (const id of ['mancuerna', 'kettlebell', 'barra', 'disco', 'bumper', 'rack', 'barra_dominadas', 'anillas', 'cajon', 'balon_medicinal', 'comba']) {
  test(`T1–T3 standard Box grants ${id} with derived provenance`, () => {
    assert.deepEqual(plain(state(box, id)), { state: 'available', source: 'derived:training_environment:v2:STANDARD_BOX:usuarios.perfil.lugar_entreno', updatedAt: null });
  });
}
...
test('Box grants conventional ergometers but not specialized strongman equipment', () => {
  for (const id of ['ghd', 'ski_erg', 'assault_bike', 'echo_bike', 'remo', 'sled', 'cuerda']) assert.equal(state(box, id).state, 'available');
  assert.equal(state(box, 'yoke').state, 'unknown');
...
test('T9/T10 STANDARD_GYM is exactly the requested existing equipment', () => {
  ...
  const expected = catalog.equipmentIds.filter(id => catalog.isEquipmentAvailableByEnvironment(id, 'GYM'));
  assert.deepEqual(plain(environment(gym).implicitEquipmentIds), plain(expected));
  for (const id of expected) assert.equal(state(gym, id).state, 'available');
});
test('T11 Gym does not grant box-specific equipment', () => {
  for (const id of ['anillas', 'bumper', 'ghd', 'sled', 'yoke', 'ski_erg', 'echo_bike', 'assault_bike']) assert.equal(state(gym, id).state, 'unknown');
});
...
test('T17 explicit persistent and material evidence retain precedence and provenance', () => {
  ...
  assert.equal(state({ ...box, material: ['Mancuernas'] }, 'mancuerna').source, 'derived:training_environment:v2:STANDARD_BOX:usuarios.perfil.lugar_entreno');
