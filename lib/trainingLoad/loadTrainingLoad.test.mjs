...
function database(overrides={},failure){
  const tables={usuarios:{workout_history:[],ciclo_actual:{bloque:'Base'}},weekly_plan:[],external_training_records:[],session_modification_events:[],...overrides},calls=[];
  return {calls,from(table){
    ...
    return {select(){return this;},eq(k,v){call.filters.push([k,v]);return this;},gte(){return this;},lte(){return this;},order(){return this;},range(a,b){start=a;end=b;call.range=[a,b];return this;},single:async()=>response(),then:(yes,no)=>Promise.resolve(response()).then(yes,no)};
  }};
}
...
for(const mode of ['coach','focus','supervision'])test(`shared route reads load in ${mode} without prescribing or writing`,async()=>{
  ...
  const result=await execute();assert.equal(result.ok,true);assert.equal(result.report.schemaVersion,1);assert.equal(db.calls.length,4);
});
